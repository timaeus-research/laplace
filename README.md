...
| [`Laplace/Patterning/ScoreFunction.lean`](Laplace/Patterning/ScoreFunction.lean) | **The score-function identity for reweighting by resampling.** For minibatch indices drawn independently from `q(1 + εω)` with `∑ q = 1`, `∑ qω = 0`, and any functional `φ` of the index path, `d/dε E_ε[φ]|₀ = E_0[φ ∑ₖ ω(sₖ)] = ∑ᵢ ω(i) Cov_0(φ, Nᵢ)` with `Nᵢ` the sample counts (`score_deriv_eq_cov`, uniform case `score_deriv_eq_cov_uniform`); normalisation `∑ₛ P_ε(s) = (1 + ε∑qω)^T` and `E_0[Nᵢ] = T q(i)`. The exact discrete counterpart of the Girsanov response (Prop. 6.2) for resampling, with no differentiability in `w` and no fixed-diffusion approximation: the path-space susceptibility is a covariance with the sample counts over ordinary runs |
| [`Laplace/Patterning/HomogeneousGibbs.lean`](Laplace/Patterning/HomogeneousGibbs.lean) | **The Gibbs law of a homogeneous loss** `K = a‖z‖^p` on `ℝ²`, `a, p > 0`: `Z(t) = 2π(ta)^{-2/p}Γ(2/p)/p`, `E_t[‖z‖^k] = (ta)^{-k/p}Γ((k+2)/p)/Γ(2/p)` for `k > −2`, and `t E_t[K] = t² Var_t[K] = 2/p` for every `t > 0` (`homogeneous_gibbs_excess`, `homogeneous_gibbs_variance`): `tK` is `Gamma(2/p, 1)`, the `d_s/p` law of the note's Gamma-sphere proposition in dimension two, generalising the 4-gon's `1/2` (`p = 4`, `a = 1/15`) |
| [`Laplace/Patterning/TiltIdentity.lean`](Laplace/Patterning/TiltIdentity.lean) | **Response is covariance with the score.** On a finite sample space, for any family `p_ε` with `d/dε p_ε(x)|₀ = p_0(x) S(x)` that stays normalised, `d/dε ∑ φ p_ε|₀ = ∑ p_0 (φ − E_0φ) S = Cov_0(φ, S)` (`deriv_expect_eq_cov`); the exponential tilt `p_ε ∝ p_0 e^{εS}` has score `S − E_0 S`, so its response is `Cov_0(φ, S)` (`tilt_deriv_eq_cov`; the posterior reweighted on sample `i` is `S = −βℓᵢ`); the resampled path law is an instance with score `∑ₖ ω(sₖ)` (`pathExpect_deriv_of_score`); and the count covariance is a sum of conditional influences `Cov_0(φ, Nᵢ) = ∑ₖ (E_0[φ·1[sₖ = i]] − q(i)E_0 φ) = ∑ₖ q(i)(E_0[φ | sₖ = i] − E_0 φ)` (`baseCov_count_eq_sum`, `baseCov_count_eq_sum_cond`), the exact form of the curriculum kernel |
