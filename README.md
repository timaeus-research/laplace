...
| [`Laplace/Patterning/OrnsteinUhlenbeck.lean`](Laplace/Patterning/OrnsteinUhlenbeck.lean) | The Ornstein–Uhlenbeck model of SGD as its Gaussian transition semigroup `N(e^{-sH} m, Σ_s)`, `Σ_s = Σ − e^{-sH} Σ e^{-sH}`: the Lyapunov ODE `Σ_s' = D − HΣ_s − Σ_s H`, the semigroup identity, `Σ_s ⪰ 0`, invariance of `N(0, Σ)` for `HΣ + ΣH = D`, the law from the minimiser, relaxation `e^{-sH} → 0`, `Σ_s → Σ` and `Σ ⪰ 0` for positive definite `H` (Prop. 12.2; the Itô SDE itself is not formalised) |
