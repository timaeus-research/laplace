...
constructive-recovery thread on the seabed whose closed forms
(\texttt{partitionFunction\_kthPotential} and the
...
\texttt{mul\_left\_cancel₀}/\texttt{mul\_right\_cancel₀}). The
instructive one: dropping the \emph{unused} hypothesis
$0 < \alpha_2$ from the rigidity lemma strengthened the statement but
left $\alpha_2, \beta_2$ unconstrained at the application site, and the
elaborator unified them with the left-hand values before the proof body
could pin them, producing a vacuous goal. The repair is to pass the
right-hand data explicitly (\texttt{(α₂ := ...) (β₂ := ...)}) when the
only anchor hypothesis mentions one side.
