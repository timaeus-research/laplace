...
quadratic Gaussian vanishes is identically zero
(\texttt{homogeneous\_eq\_zero\_of\_gaussianCovariance\_self\_eq\_zero}).
...
pre-flight warning to check whether \texttt{quadKernel\_integrable\_pow}
really is the norm-moment form (it is) saved a potential wrong turn.

\section{Mathlib lookup versus new mathematics}

Lookup: ball positivity and set-integral comparisons
(\texttt{measure\_ball\_pos}, \texttt{setIntegral\_ge\_of\_const\_le},
\texttt{setIntegral\_le\_integral}) and the H2 integrability
theorems. New (project-level): the growth certificate and its
