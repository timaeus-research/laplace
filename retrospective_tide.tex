...
\title{Tide-loop retrospective:\\three formalisations and a side
excursion\\in a single afternoon}
...
\begin{abstract}
This document is a retrospective on three Tide-loop excursions and one
adjacent numerical/theoretical exploration carried out in a single
afternoon, building on the morning recorded in
\texttt{retrospective.tex} and the multi-week saga in
...
\texttt{lean-formalisation} skill until success. Three loops produced
1099 lines of new Lean across three branches of the
\texttt{timaeus-research/laplace} repository, with zero \texttt{sorry} /
\texttt{axiom} / \texttt{native\_decide}, plus a numerical exploration
note with a Python script and three figures, plus three new appendix
sections in the susceptibility primer's annotated copy. The aim of this
retrospective is to record what worked about the loop, where it was
stress-tested by repeated use, and what skill iteration emerged from
running it three times in succession.
\end{abstract}
...
Lean. The multi-week saga that followed, recorded in
...
each time). The rest of this document records what the runs looked
like, how the deliberation step calibrated, where the skill was
stress-tested, and what was learned.

\section{The three excursions}\label{sec:excursions}
...
\toprule
\textbf{Tide step} & \textbf{Direction} & \textbf{Branch} & \textbf{Lines} & \textbf{Lake build}\\
\midrule
1. quartic-moments        & ``extend the formalisation to simple degenerate cases''    & \texttt{tide/quartic-moments}        & 379 & 4s\\
2. 2d-semi-degenerate     & ``Candidate D from the previous deliberation'' (i.e.\ ``Next'')          & \texttt{tide/2d-semi-degenerate}     & 514 & 12s\\
3. universal-scaling      & ``formalise the universal scaling formula''                & \texttt{tide/universal-scaling}      & 206 & 8s\\
\midrule
\multicolumn{3}{r}{\emph{Total new Lean for tide loop}} & \textbf{1099} & \\
\bottomrule
\end{tabular}
\caption{The three Tide-loop excursions of 1 May 2026, with branch
names rebased linearly: each step's seabed is the previous step's tip.}
...
The skill prescribes opening a tide log file
(\texttt{sri/projects/automation/log/2026-05-01-tide-degenerate-quartic-cov.md})
...
The investigation took $\sim 30$ minutes. The result --- recorded in
\texttt{sri/projects/automation/log/2026-05-01-near-degenerate-crossover.md}
...
\paragraph{Fold all three branches to \texttt{main}.} The branches are
linear and individually pass \texttt{lake build}; the merges to
\texttt{main} are clean. This collapses the primer's
\texttt{\textbackslash quarticCommit} pin into the standard
\texttt{\textbackslash laplaceCommit} and removes the temporary
\texttt{\textbackslash leanrefQuartic} macro.

\paragraph{Tide 4 and beyond.} Three obvious continuations from the
present shoreline:
\begin{itemize}
\item Two-sided asymptotic for $f_n(T)$ in the universal potential
(Candidate (2) from the near-degenerate note); requires a uniform
Taylor expansion against the quartic Gibbs at small $T$ and a standard
Laplace expansion at large $T$.
\item A crossover-aware uniform-in-$\varepsilon$ inequality
(Candidate (3) from the near-degenerate note); a paper-level result.
\item A 2D analogue of the universal scaling identity, combining the
1D scaling of Tide 3 with the product-measure structure of Tide 2.
\end{itemize}
...
end of this afternoon there are five formalised primer claims (two
from the morning of 27 April, plus the three from this afternoon:
the 1D quartic affine covariance, the 2D semi-degenerate affine
covariance, and the universal scaling identity\footnote{Numbered
\texttt{lem:quartic-cov-affine}, \texttt{lem:semi-degenerate-cov-affine},
and \texttt{lem:nd-scaling} in the primer copy compiled from
\texttt{papers/suscprimer/main\_lean.tex}.}), with margin markers and
two separate commit pins. \texttt{leanblueprint}'s dependency-graph
view would now be genuinely useful, and the manual-pin bookkeeping
would go away.

\section{Acknowledgements}

\begin{itemize}
\item GPT-5.5~Pro provided three deliberation consultations preserved
verbatim under \texttt{sri/projects/automation/log/} as
\texttt{gpt55\_tide\_quartic\_v1.md},
\texttt{gpt55\_tide\_2d\_v1.md}, and
