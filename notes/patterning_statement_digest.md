...
| Prop (Gamma law), `t² Var_t[K] = λ = ½` for the dead component; `d/2 + O(1/t)` for a regular minimum | `deadQuartic_gibbs_variance` (FourGonSusceptibilityRadial.lean), `deadQuartic_gibbs_excess`; `varV_first_order_rate_posDef` (Multi/CovKClosedForm.lean) | quartic Gibbs law, `t > 0`; regular case: quintic-jet potential with `P ≻ 0` | the general Gamma law for homogeneous `K` on `ℝᵈ` is proved in the text only |
...
| Sec 16.4 (one identity, three measures): response = covariance with the score, any normalised differentiable family on a finite sample space | `hasDerivAt_expect_score`, `sum_score_eq_zero`, `deriv_expect_eq_cov` (TiltIdentity.lean) | `∀ x, HasDerivAt (p · x) (p 0 x * S x) 0`, `∀ ε, ∑ p ε x = 1` | infinite sample spaces (measure theory) |
| Sec 16.4, exponential tilt `p_ε ∝ p_0 e^{εS}`: score = centred statistic, response = `Cov_0(φ, S)` | `tilt`, `tiltNorm_pos`, `sum_tilt`, `hasDerivAt_tilt`, `tilt_deriv_eq_cov` | `∀ x, 0 ≤ p₀ x`, `∑ p₀ = 1` | the posterior instance `S = −βℓᵢ` is the primer's formalised static fluctuation-response relation, not re-derived here |
| Sec 16.4, the path law as an instance: `d/dε E_ε[φ]|₀ = ∑ₛ P_0(s)(φ(s) − E_0φ) ∑ₖ ω(sₖ)` | `pathExpect_deriv_of_score` | `∑ q = 1`, `∑ qω = 0` | |
| Sec 16.4, conditional-influence form `Cov_0(φ, Nᵢ) = ∑ₖ (E_0[φ·1[sₖ=i]] − q(i)E_0φ) = ∑ₖ q(i)(E_0[φ | sₖ=i] − E_0φ)` | `count_eq_sum_ind`, `baseExpect_mul_count`, `baseCov_count_eq_sum`, `baseCov_count_eq_sum_cond` | `∑ q = 1` (and `q i ≠ 0` for the conditional form) | the linearisation of each term to the propagated gradient (curriculum kernel) |
