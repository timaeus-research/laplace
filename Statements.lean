...
The headline results of this repository, stated self-contained: everything
they mention is Mathlib vocabulary, a definition in this file, or a definition
in `Laplace/Multi/Defs.lean`. The statements are deliberately left as `sorry`
here; each is proved in `Solutions.lean` by delegation to the corresponding
library theorem.

* `cov_anharmonic_asymptotic` — for the 1D anharmonic potential
  `L(x) = (λ/2)x² + (α/6)x³ + (γ/24)x⁴`, the Gibbs covariance satisfies
  `t² · Cov_t[x², x] → -2α/λ³` as `t → ∞`.
* `kappa3_anharmonic_asymptotic` — the third cumulant of the same measure
  satisfies `t² · κ₃(x, x, x) → -α/λ³`.
* `gibbsCov_deriv_anharmonic_asymptotic` — the cross-susceptibility
  (the field-derivative of the perturbed self-covariance) satisfies
  `t · ∂_h Cov_h(x, x)|₀ → α/λ³`.
* `gibbsCov_first_order_rate_sharp` — multivariate leading-order covariance:
  `Cov_t[φ, ψ] = (1/t)·⟨∇φ(0), H⁻¹ ∇ψ(0)⟩ + O(t⁻²)`.
* `gibbsExpectation_first_order_rate_explicit` — explicit first-order
  expectation: `⟨φ⟩_t = (1/2t)·[tr(AΣ) - (Σ∇φ(0))·(T:Σ)] + O(t⁻²)`.
* `gibbsCov_first_order_rate_explicit` — explicit next-to-leading covariance:
  for `∇φ(0) = 0`, `t² · Cov_t[φ, ψ]` converges at rate `O(1/t)` to the
  four-term tensor-contraction coefficient `cov2Coeff`.
* `gibbsExpectation_kthKth_isEquivalent_rpow` — for the degenerate separable
  2D potential `x^(2k₁)/(2k₁)! + y^(2k₂)/(2k₂)!`, the mixed even moment
  `⟨x^(2j₁) y^(2j₂)⟩_t` is asymptotically equivalent to an explicit
  Gamma-quotient constant times `t^(-j₁/k₁ - j₂/k₂)`.
-/
