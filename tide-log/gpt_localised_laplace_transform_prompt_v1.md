You are consulted (one round) on tide 89 of an automated Lean 4 + Mathlib formalisation loop extending the `laplace` seabed
...
parameters, eventual remainders (`∃ K T, ∀ t ≥ T, |…| ≤ K/t`). Tides 85–88 (your consults approved them) landed the first four cumulants
of the localised energy on E2's exact localised measure with the Gamma(d/2, rate t) leading asymptotics; your tide-88 consult stressed that
four cumulants do not identify a law. This tide goes after the law itself, through its Laplace transform.
