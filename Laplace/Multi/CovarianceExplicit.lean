...
/-- **Tail pointwise bound on the Gaussian-symmetric integrand**
(helper for `bulkErrA_gaussian_asymptotic`).

For `‖u‖ > δ·√t` (with δ = jet_radius_φ),
`|t·√t · (b·u) · (r(u) - r(-u)) · gW(u)| ≤ K_tail · (1+‖u‖^M) ·
  exp(-(c/4)·‖u‖²) · exp(-(c·δ²/4)·t)`.

Uses global polynomial bound on `r(±u)` via `abs_expNumObsRem_global_le`,
tail trick `t·√t ≤ ‖u‖³/δ³`, and exp split using
`(c/2)‖u‖² = (c/4)‖u‖² + (c/4)‖u‖²` ≥ (c/4)‖u‖² + (c·δ²/4)·t in tail.
Pattern mirrors `abs_bulkErrA_mul_gW_mul_exp_sub_one_tail_le` but with
`gW` (no exp(-s_t)) factor. -/
private lemma abs_bulkErrA_gaussian_symm_tail_le
    (V φ : (ι → ℝ) → ℝ)
    (H : (ι → ℝ) →L[ℝ] (ι → ℝ))
    (b : ι → ℝ)
    [Nonempty ι]
    ...
    ∃ K_tail : ℝ, ∃ M : ℕ, 0 ≤ K_tail ∧ ∀ t : ℝ, 1 ≤ t →
      ∀ u : ι → ℝ, hφ.toObservableTensorApprox.jet_radius * Real.sqrt t < ‖u‖ →
        |t * Real.sqrt t * dot b u *
            (expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
              - expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)) *
            gaussianWeight H u|
          ≤ K_tail * (1 + ‖u‖ ^ M) *
              Real.exp (-((hV.coercive_const / 4) * ‖u‖ ^ 2)) *
              Real.exp (-((hV.coercive_const *
                hφ.toObservableTensorApprox.jet_radius ^ 2 / 4) * t)) := by
  classical
  ...
  have hR_const_nn : 0 ≤ R_const := by rw [hR_const_def]; positivity
  set bL1 : ℝ := ∑ i, |b i| with hbL1_def
  have hbL1_nn : 0 ≤ bL1 := Finset.sum_nonneg (fun _ _ => abs_nonneg _)
  set δ : ℝ := hφ.toObservableTensorApprox.jet_radius with hδ_def
  have hδ_pos : 0 < δ := hφ.toObservableTensorApprox.jet_radius_pos
  have hδ3_pos : 0 < δ ^ 3 := pow_pos hδ_pos 3
  set K_tail : ℝ := 4 * bL1 * R_const / δ ^ 3 with hK_tail_def
  have hK_tail_nn : 0 ≤ K_tail := by rw [hK_tail_def]; positivity
  ...
  have h_t_sqt_nn : 0 ≤ t * Real.sqrt t := by positivity
  have h_gW_nn : 0 ≤ gaussianWeight H u := (gaussianWeight_pos H u).le
  -- Polynomial bound on |expNumObsRem|.
  have h_R_global : ∀ v : ι → ℝ,
      |expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t v|
        ≤ R_const * (1 + ‖v‖ ^ N) := by
    intro v
    ...
    calc |expNumObsRem φ 0 hφ.toObservableTensorApprox t v|
        ≤ Kφ * (1 + ‖v‖ ^ p)
            + (1 / 2 : ℝ) * ↑(Fintype.card ι) *
                ‖hφ.toObservableTensorApprox.A‖ * ‖v‖ ^ 2
            + ‖hφ.toObservableTensorApprox.Φ‖ / 6 * ‖v‖ ^ 3 := h
      _ ≤ Kφ * (1 + (1 + ‖v‖ ^ N))
            + (1 / 2 : ℝ) * N₁ * ‖hφ.toObservableTensorApprox.A‖ *
                (1 + ‖v‖ ^ N)
  ...
  have h_dot : |dot b u| ≤ bL1 * ‖u‖ := by
    rw [hbL1_def]; exact abs_dot_le_l1_mul_norm b u
  -- |r(u) - r(-u)| ≤ 2·R·(1+‖u‖^N).
  have h_diff_le : |expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
        - expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)|
      ≤ 2 * (R_const * (1 + ‖u‖ ^ N)) := by
    have h1 := h_R_global u
    have h2 := h_R_global (-u)
    rw [show ‖(-u : ι → ℝ)‖ = ‖u‖ from norm_neg _] at h2
    calc |expNumObsRem φ 0 hφ.toObservableTensorApprox t u
          - expNumObsRem φ 0 hφ.toObservableTensorApprox t (-u)|
        ≤ |expNumObsRem φ 0 hφ.toObservableTensorApprox t u|
          + |expNumObsRem φ 0 hφ.toObservableTensorApprox t (-u)| := abs_sub _ _
      _ ≤ R_const * (1 + ‖u‖ ^ N) + R_const * (1 + ‖u‖ ^ N) := by linarith
      _ = 2 * (R_const * (1 + ‖u‖ ^ N)) := by ring
  -- gW(u) ≤ exp(-c/2·‖u‖²).
  have h_gW_le : gaussianWeight H u ≤ Real.exp (-((c / 2) * ‖u‖ ^ 2)) :=
    gaussianWeight_le_exp_neg_coercive V H hV.toPotentialTensorApprox u
  -- Tail trick: t·√t·δ³ ≤ ‖u‖³.
  have h_t_sqt : t * Real.sqrt t * δ ^ 3 ≤ ‖u‖ ^ 3 := by
    have h_sqt_le : Real.sqrt t < ‖u‖ / δ := by
      rw [lt_div_iff₀ hδ_pos]; linarith
    have h_sqrt_sq : Real.sqrt t * Real.sqrt t = t := Real.mul_self_sqrt ht_pos.le
    calc t * Real.sqrt t * δ ^ 3
        = (Real.sqrt t * Real.sqrt t) * Real.sqrt t * δ ^ 3 := by rw [h_sqrt_sq]
      _ ≤ (‖u‖ / δ) * (‖u‖ / δ) * (‖u‖ / δ) * δ ^ 3 := by
          have h_div_nn : 0 ≤ ‖u‖ / δ := div_nonneg h_norm_nn hδ_pos.le
          have h1 : Real.sqrt t * Real.sqrt t ≤ (‖u‖ / δ) * (‖u‖ / δ) :=
            mul_le_mul h_sqt_le.le h_sqt_le.le hsqrt_pos.le h_div_nn
          have h2 : Real.sqrt t * Real.sqrt t * Real.sqrt t
              ≤ (‖u‖ / δ) * (‖u‖ / δ) * (‖u‖ / δ) :=
            mul_le_mul h1 h_sqt_le.le hsqrt_pos.le (by positivity)
          exact mul_le_mul_of_nonneg_right h2 (by positivity)
      _ = ‖u‖ ^ 3 := by field_simp
  -- exp(-c/2·‖u‖²) ≤ exp(-c/4·‖u‖²)·exp(-cδ²/4·t) on tail.
  ...
  --       ≤ (‖u‖^3/δ³) · bL1·‖u‖ · 2R·(1+‖u‖^N) · exp(-c/4·‖u‖²)·exp(-cδ²/4·t)
  --       ≤ (4·bL1·R/δ³)·(1+‖u‖^(N+4))·exp(-c/4·‖u‖²)·exp(-cδ²/4·t)
  have h_abs_eq : |t * Real.sqrt t * dot b u *
        (expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
          - expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)) *
        gaussianWeight H u|
    ...
    abs_mul4_of_nonneg h_t_sqt_nn h_gW_nn
  rw [h_abs_eq]
  -- Step 1: bound |dot| · |r-r| via the factor inequalities.
  have h_dot_q := mul_le_mul h_dot h_diff_le (abs_nonneg _)
    (mul_nonneg hbL1_nn h_norm_nn)
  have h_step1 := mul_le_mul_of_nonneg_left h_dot_q h_t_sqt_nn
  have h_assoc1 : t * Real.sqrt t * |dot b u| *
          |expNumObsRem φ 0 hφ.toObservableTensorApprox t u
            - expNumObsRem φ 0 hφ.toObservableTensorApprox t (-u)|
        = t * Real.sqrt t * (|dot b u| *
            |expNumObsRem φ 0 hφ.toObservableTensorApprox t u
              - expNumObsRem φ 0 hφ.toObservableTensorApprox t (-u)|) := by ring
  have h_step1' : t * Real.sqrt t * |dot b u| *
        |expNumObsRem φ 0 hφ.toObservableTensorApprox t u
          - expNumObsRem φ 0 hφ.toObservableTensorApprox t (-u)|
      ≤ t * Real.sqrt t * (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N)))) := by
    rw [h_assoc1]; exact h_step1
  -- Multiply by gW.
  have h_step2 : t * Real.sqrt t * |dot b u| *
          |expNumObsRem φ 0 hφ.toObservableTensorApprox t u
            - expNumObsRem φ 0 hφ.toObservableTensorApprox t (-u)| *
          gaussianWeight H u
      ≤ t * Real.sqrt t * (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N)))) *
          gaussianWeight H u :=
    mul_le_mul_of_nonneg_right h_step1' h_gW_nn
  -- Replace gW by exp(-c/2·‖u‖²).
  have h_factor_nn : 0 ≤ t * Real.sqrt t *
      (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N)))) := by positivity
  have h_step3 : t * Real.sqrt t *
        (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N)))) *
        gaussianWeight H u
      ≤ t * Real.sqrt t * (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N)))) *
          Real.exp (-((c / 2) * ‖u‖ ^ 2)) :=
    mul_le_mul_of_nonneg_left h_gW_le h_factor_nn
  -- Apply tail trick + exp split. End at 4·bL1·R/δ³ · (1+‖u‖^(N+4)) · exp(...)
  -- Tail trick: t·√t·‖u‖ ≤ ‖u‖^4/δ³.
  have h_t_sqt_norm : t * Real.sqrt t * ‖u‖ ≤ ‖u‖ ^ 4 / δ ^ 3 := by
    rw [le_div_iff₀ hδ3_pos]
  ...
  have h_pow_combine : ‖u‖ ^ 4 * (1 + ‖u‖ ^ N) ≤ 2 * (1 + ‖u‖ ^ (N + 4)) := by
    ...
    calc ‖u‖ ^ 4 * (1 + ‖u‖ ^ N)
        = ‖u‖ ^ 4 + ‖u‖ ^ 4 * ‖u‖ ^ N := by ring
      _ = ‖u‖ ^ 4 + ‖u‖ ^ (N + 4) := by rw [h_pow_eq]
      _ ≤ 2 * (1 + ‖u‖ ^ (N + 4)) := by linarith
  -- Now combine: t·√t·bL1·‖u‖ · 2R(1+‖u‖^N) ≤ 2·bL1·R · (t·√t·‖u‖)·(1+‖u‖^N)
  --   ≤ 2·bL1·R · (‖u‖^4/δ³) · (1+‖u‖^N)
  --   = (2·bL1·R/δ³) · ‖u‖^4·(1+‖u‖^N)
  --   ≤ (2·bL1·R/δ³) · 2·(1+‖u‖^(N+4)) = K_tail · (1+‖u‖^(N+4)).
  have h_combine_pol : t * Real.sqrt t *
        (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N))))
      ≤ K_tail * (1 + ‖u‖ ^ (N + 4)) := by
    have h_eq1 : t * Real.sqrt t * (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N))))
        = 2 * (bL1 * R_const) * (t * Real.sqrt t * ‖u‖) * (1 + ‖u‖ ^ N) := by ring
    rw [h_eq1]
    have h_2bR_nn : 0 ≤ 2 * (bL1 * R_const) := by positivity
    have h_pow_nn : 0 ≤ 1 + ‖u‖ ^ N := by positivity
    have h_step_t : (t * Real.sqrt t * ‖u‖) * (1 + ‖u‖ ^ N)
        ≤ (‖u‖ ^ 4 / δ ^ 3) * (1 + ‖u‖ ^ N) :=
      mul_le_mul_of_nonneg_right h_t_sqt_norm h_pow_nn
    have h_step_pow : (‖u‖ ^ 4 / δ ^ 3) * (1 + ‖u‖ ^ N)
        ≤ 2 / δ ^ 3 * (1 + ‖u‖ ^ (N + 4)) := by
      have h_inv_pos : 0 < 1 / δ ^ 3 := by positivity
      calc (‖u‖ ^ 4 / δ ^ 3) * (1 + ‖u‖ ^ N)
          = (1 / δ ^ 3) * (‖u‖ ^ 4 * (1 + ‖u‖ ^ N)) := by ring
        _ ≤ (1 / δ ^ 3) * (2 * (1 + ‖u‖ ^ (N + 4))) :=
            mul_le_mul_of_nonneg_left h_pow_combine h_inv_pos.le
        _ = 2 / δ ^ 3 * (1 + ‖u‖ ^ (N + 4)) := by ring
    have h_assoc2 : 2 * (bL1 * R_const) * (t * Real.sqrt t * ‖u‖) * (1 + ‖u‖ ^ N)
        = 2 * (bL1 * R_const) *
            ((t * Real.sqrt t * ‖u‖) * (1 + ‖u‖ ^ N)) := by ring
    rw [h_assoc2]
    have h_combine := mul_le_mul_of_nonneg_left
      (le_trans h_step_t h_step_pow) h_2bR_nn
    have h_K_eq : 2 * (bL1 * R_const) * (2 / δ ^ 3 * (1 + ‖u‖ ^ (N + 4)))
        = K_tail * (1 + ‖u‖ ^ (N + 4)) := by rw [hK_tail_def]; ring
    linarith [h_combine, h_K_eq.le, h_K_eq.ge]
  -- Apply the polynomial bound multiplied by exp(-c/2·‖u‖²).
  have h_exp_nn : 0 ≤ Real.exp (-((c / 2) * ‖u‖ ^ 2)) := (Real.exp_pos _).le
  have h_K_pol_nn : 0 ≤ K_tail * (1 + ‖u‖ ^ (N + 4)) :=
    mul_nonneg hK_tail_nn (by positivity)
  have h_step4 : t * Real.sqrt t *
        (bL1 * ‖u‖ * (2 * (R_const * (1 + ‖u‖ ^ N)))) *
        Real.exp (-((c / 2) * ‖u‖ ^ 2))
      ≤ K_tail * (1 + ‖u‖ ^ (N + 4)) * Real.exp (-((c / 2) * ‖u‖ ^ 2)) :=
    mul_le_mul_of_nonneg_right h_combine_pol h_exp_nn
  -- Apply exp split.
  have h_step5 : K_tail * (1 + ‖u‖ ^ (N + 4)) * Real.exp (-((c / 2) * ‖u‖ ^ 2))
      ≤ K_tail * (1 + ‖u‖ ^ (N + 4)) *
          (Real.exp (-((c / 4) * ‖u‖ ^ 2)) *
            Real.exp (-((c * δ ^ 2 / 4) * t))) :=
    mul_le_mul_of_nonneg_left h_exp_split h_K_pol_nn
  have h_assoc_final : K_tail * (1 + ‖u‖ ^ (N + 4)) *
      (Real.exp (-((c / 4) * ‖u‖ ^ 2)) *
        Real.exp (-((c * δ ^ 2 / 4) * t)))
      = K_tail * (1 + ‖u‖ ^ (N + 4)) *
          Real.exp (-((c / 4) * ‖u‖ ^ 2)) *
          Real.exp (-((c * δ ^ 2 / 4) * t)) := by ring
  linarith [h_step2, h_step3, h_step4, h_step5,
    h_assoc_final.le, h_assoc_final.ge]
...
/-- **Tail pointwise bound on the perturbative bulk integrand** (per GPT
alt-path: `gpt_responses/strategy_stage5_alt_path.md`).

For `‖u‖ > δ·√t`,
`|B_t(u) · gW(u) · (exp(-s_t(u)) - 1)| ≤ K_tail · (1 + ‖u‖^M) ·
  exp(-(c/4)·‖u‖²) · exp(-(c·δ²/4)·t)`.

The `exp(-(c·δ²/4)·t)` factor decays faster than any `1/t^k`, so this
integrates to `o(1/t)` in the tail.

Uses the global polynomial bound `abs_expNumObsRem_global_le` together
with the perturbative factor bound `abs_gaussianWeight_mul_exp_sub_one_le_tail`
and the tail trick `t·√t ≤ ‖u‖³/δ³`. -/
private lemma abs_bulkErrA_mul_gW_mul_exp_sub_one_tail_le
    (V φ : (ι → ℝ) → ℝ)
    (H : (ι → ℝ) →L[ℝ] (ι → ℝ))
    (b : ι → ℝ)
    [Nonempty ι]
    ...
    {δ : ℝ} (hδ_pos : 0 < δ) :
    ∃ K_tail : ℝ, ∃ M : ℕ, 0 ≤ K_tail ∧ ∀ t : ℝ, 1 ≤ t →
      ∀ u : ι → ℝ, δ * Real.sqrt t < ‖u‖ →
        |bulkErrA φ b hφ.toObservableTensorApprox t u *
            gaussianWeight H u *
            (Real.exp (-(rescaledPerturbation V H t u)) - 1)|
          ≤ K_tail * (1 + ‖u‖ ^ M) *
              Real.exp (-((hV.coercive_const / 4) * ‖u‖ ^ 2)) *
              Real.exp (-((hV.coercive_const * δ ^ 2 / 4) * t)) := by
  classical
  obtain ⟨Kφ, p, hKφ_nn, h_poly_φ⟩ := hφ.toObservableApprox.poly_growth
  set N : ℕ := max p 3 with hN_def
  set N₁ : ℝ := (Fintype.card ι : ℝ) with hN1_def
  ...
  have h_norm_pos : 0 < ‖u‖ := by
    have h1 : 0 < δ * Real.sqrt t := mul_pos hδ_pos hsqrt_pos
    linarith
  set c : ℝ := hV.coercive_const with hc_def
  have hc_pos : 0 < c := hV.toPotentialApprox.coercive_const_pos
  have h_coer := hV.toPotentialApprox.coercive_bound
  ...
  have h_dot_b : |dot b u| ≤ bL1 * ‖u‖ := by
    rw [hbL1_def]; exact abs_dot_le_l1_mul_norm b u
  -- |B_t(u)| ≤ t·√t·bL1·‖u‖·R_const·(1+‖u‖^N).
  have h_B_t_global : |bulkErrA φ b hφ.toObservableTensorApprox t u|
    ...
    rw [show t * Real.sqrt t * dot b u *
          ...
          = (t * Real.sqrt t) * (dot b u *
              expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u)
        from by ring,
        abs_mul, abs_of_pos (by positivity : 0 < t * Real.sqrt t), abs_mul]
    have h_t_nn : 0 ≤ t * Real.sqrt t := by positivity
    have h_dot_R := mul_le_mul h_dot_b (h_R_global u) (abs_nonneg _)
      (mul_nonneg hbL1_nn h_norm_nn)
    have h_step := mul_le_mul_of_nonneg_left h_dot_R h_t_nn
    have h_eq : t * Real.sqrt t * (bL1 * ‖u‖ * (R_const * (1 + ‖u‖ ^ N)))
        = t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) := by ring
    linarith
  -- Tail factor bound on `gW · (exp(-s_t) - 1)`.
  have h_gW_exp_tail := abs_gaussianWeight_mul_exp_sub_one_le_tail
    V H hc_pos hV.local_radius_pos hCs_nn h_coer hV.local_bound
    hδ_pos ht_pos u hu_tail
  -- Tail trick: t·√t ≤ ‖u‖^3/δ^3.
  have h_t_sqt : t * Real.sqrt t ≤ ‖u‖ ^ 3 / δ ^ 3 := by
    rw [le_div_iff₀ hδ3_pos]
    have h_sqt_le : Real.sqrt t < ‖u‖ / δ := by
      rw [lt_div_iff₀ hδ_pos]; linarith
    have h_sqrt_sq : Real.sqrt t * Real.sqrt t = t :=
      Real.mul_self_sqrt ht_pos.le
    have h_t_eq : t = Real.sqrt t * Real.sqrt t := h_sqrt_sq.symm
    calc t * Real.sqrt t * δ ^ 3
        = Real.sqrt t * Real.sqrt t * Real.sqrt t * δ ^ 3 := by rw [← h_t_eq]
      _ ≤ (‖u‖ / δ) * (‖u‖ / δ) * (‖u‖ / δ) * δ ^ 3 := by
          have h_div_nn : 0 ≤ ‖u‖ / δ := div_nonneg h_norm_nn hδ_pos.le
          have h1 : Real.sqrt t * Real.sqrt t ≤ (‖u‖ / δ) * (‖u‖ / δ) :=
            mul_le_mul h_sqt_le.le h_sqt_le.le hsqrt_pos.le h_div_nn
      ...
      _ = ‖u‖ ^ 3 := by field_simp
  -- |B_t| · ‖u‖ ≤ ?
  have h_B_t_tail : |bulkErrA φ b hφ.toObservableTensorApprox t u|
      ≤ (bL1 * R_const / δ ^ 3) * (‖u‖ ^ 4 + ‖u‖ ^ (N + 4)) := by
    have h_mul_step : t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N))
        ≤ (‖u‖ ^ 3 / δ ^ 3) * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) := by
      have h_factor_nn : 0 ≤ bL1 * ‖u‖ * (R_const * (1 + ‖u‖ ^ N)) := by positivity
      have h := mul_le_mul_of_nonneg_right h_t_sqt h_factor_nn
      linarith
    have h_simp :
        (‖u‖ ^ 3 / δ ^ 3) * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N))
          = (bL1 * R_const / δ ^ 3) * (‖u‖ ^ 4 + ‖u‖ ^ (N + 4)) := by
      have h_pow : ‖u‖ ^ (N + 4) = ‖u‖ ^ 4 * ‖u‖ ^ N := by
        rw [show (N + 4 : ℕ) = 4 + N from by ring, pow_add]
      rw [show ‖u‖ ^ 4 + ‖u‖ ^ (N + 4) = ‖u‖ ^ 4 * (1 + ‖u‖ ^ N)
          from by rw [h_pow]; ring]
  ...
  have h_eq_abs :
    ...
    rw [show bulkErrA φ b hφ.toObservableTensorApprox t u *
        ...
        abs_mul]
  rw [h_eq_abs]
  have h_term_le := mul_le_mul h_B_t_tail h_gW_exp_tail (abs_nonneg _)
    (by positivity : 0 ≤ bL1 * R_const / δ ^ 3 * (‖u‖ ^ 4 + ‖u‖ ^ (N + 4)))
  -- Show: (bL1·R_const/δ³)·(‖u‖^4+‖u‖^(N+4))·(2·exp(-c/4)·exp(-cδ²/4·t))
  --     ≤ K_tail·(1+‖u‖^(N+4))·exp(-c/4)·exp(-cδ²/4·t)
  -- using ‖u‖^4 + ‖u‖^(N+4) ≤ 2·(1+‖u‖^(N+4)) and K_tail = 4·bL1·R_const/δ³.
  have h_pow_combine : ‖u‖ ^ 4 + ‖u‖ ^ (N + 4) ≤ 2 * (1 + ‖u‖ ^ (N + 4)) := by
    have h_pow4_le : ‖u‖ ^ 4 ≤ 1 + ‖u‖ ^ (N + 4) := by
      by_cases hu1 : ‖u‖ ≤ 1
      · have : ‖u‖ ^ 4 ≤ 1 := pow_le_one₀ (norm_nonneg _) hu1
        linarith [pow_nonneg (norm_nonneg u) (N + 4)]
  ...
  have h_K_eq : K_tail = 2 * (bL1 * R_const / δ ^ 3) * 2 := by
    rw [hK_tail_def]; ring
  have h_pos_exp_factor : 0 ≤ 2 * Real.exp (-(c / 4 * ‖u‖ ^ 2)) *
      Real.exp (-(c * δ ^ 2 / 4 * t)) := by positivity
  -- (bL1·R_const/δ³) · (‖u‖^4 + ‖u‖^(N+4)) · (2·exp(...)·exp(...))
  -- ≤ (bL1·R_const/δ³) · 2·(1+‖u‖^(N+4)) · (2·exp(...)·exp(...))
  -- = 4·(bL1·R_const/δ³) · (1+‖u‖^(N+4)) · exp(...) · exp(...)
  -- = K_tail · (1+‖u‖^(N+4)) · exp(...) · exp(...).
  have h_pol_step : (bL1 * R_const / δ ^ 3) * (‖u‖ ^ 4 + ‖u‖ ^ (N + 4))
        * (2 * Real.exp (-(c / 4 * ‖u‖ ^ 2)) * Real.exp (-(c * δ ^ 2 / 4 * t)))
      ≤ K_tail * (1 + ‖u‖ ^ (N + 4)) *
          Real.exp (-(c / 4 * ‖u‖ ^ 2)) * Real.exp (-(c * δ ^ 2 / 4 * t)) := by
    have h_C_nn : 0 ≤ bL1 * R_const / δ ^ 3 := by positivity
    have h_step1 : (bL1 * R_const / δ ^ 3) * (‖u‖ ^ 4 + ‖u‖ ^ (N + 4))
        ≤ (bL1 * R_const / δ ^ 3) * (2 * (1 + ‖u‖ ^ (N + 4))) :=
      mul_le_mul_of_nonneg_left h_pow_combine h_C_nn
    have h_step2 := mul_le_mul_of_nonneg_right h_step1 h_pos_exp_factor
    have h_eq_RHS : (bL1 * R_const / δ ^ 3) * (2 * (1 + ‖u‖ ^ (N + 4)))
            * (2 * Real.exp (-(c / 4 * ‖u‖ ^ 2)) * Real.exp (-(c * δ ^ 2 / 4 * t)))
        = K_tail * (1 + ‖u‖ ^ (N + 4)) *
            Real.exp (-(c / 4 * ‖u‖ ^ 2)) * Real.exp (-(c * δ ^ 2 / 4 * t)) := by
      rw [h_K_eq]; ring
...
/-- **`bulkErrA · gW` integrability** (helper for `bulkErrA_gaussian_asymptotic`).

Uses polynomial dominance with `gW(u) ≤ exp(-(c/2)·‖u‖²)` from
`gaussianWeight_le_exp_neg_coercive`. Same shape as
`integrable_bulkErrA_mul_rescaled_weight` but using `c/2` (gW-only)
instead of `c` (gW · exp(-s_t)). -/
private lemma integrable_bulkErrA_mul_gaussianWeight
    (V φ : (ι → ℝ) → ℝ)
    (H : (ι → ℝ) →L[ℝ] (ι → ℝ))
    (b : ι → ℝ)
    [Nonempty ι]
  ...
  have hφ_cont : Continuous φ := hφ.toObservableApprox.phi_continuous
  -- Setup polynomial bound on |expNumObsRem|.
  obtain ⟨Kφ, p, hKφ_nn, h_poly_φ⟩ := hφ.toObservableApprox.poly_growth
  set N : ℕ := max p 3 with hN_def
  set N₁ : ℝ := (Fintype.card ι : ℝ) with hN1_def
  have hN1_nn : 0 ≤ N₁ := by rw [hN1_def]; exact_mod_cast Nat.zero_le _
  ...
  have h_R_global : ∀ v : ι → ℝ,
    ...
    intro v
    have h := abs_expNumObsRem_global_le (φ := φ) (a := (0 : ι → ℝ))
      (hφ := hφ.toObservableTensorApprox) hKφ_nn h_poly_φ ht v
    have h_dot_zero : ∑ i : ι, |((0 : ι → ℝ)) i| = 0 := by simp
    rw [h_dot_zero, zero_mul, add_zero] at h
    have h_normN_nn : 0 ≤ ‖v‖ ^ N := pow_nonneg (norm_nonneg _) _
    have h_p_le_N : p ≤ N := le_max_left _ _
  ...
  have h_R_cont : Continuous (fun u : ι → ℝ =>
    ...
    have h_lin_cont : Continuous (fun u : ι → ℝ =>
        (Real.sqrt t)⁻¹ * dot (0 : ι → ℝ) u) := by
      have h_eq : (fun u : ι → ℝ => (Real.sqrt t)⁻¹ * dot (0 : ι → ℝ) u)
          = fun _ => 0 := by funext u; unfold dot; simp
      rw [h_eq]; exact continuous_const
    have h_quad_cont : Continuous (fun u : ι → ℝ =>
        (1 / t) * ((1 / 2 : ℝ) *
          quadForm hφ.toObservableTensorApprox.A u)) :=
  ...
  have h_int_cont : Continuous (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u) :=
    h_bulkErrA_cont.mul (continuous_gaussianWeight H)
  -- Polynomial dominator: |B_t · gW| ≤ Cdom · ‖u‖·(1+‖u‖^N)·exp(-c/2·‖u‖²).
  set Cdom : ℝ := t * Real.sqrt t * bL1 * R_const with hCdom_def
  have hCdom_nn : 0 ≤ Cdom := by rw [hCdom_def]; positivity
  have h_int1 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 2) 1
  have h_intN1 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 2) (N + 1)
  have h_int_dom : Integrable (fun u : ι → ℝ =>
      Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2)))) := by
    have ha := h_int1.const_mul Cdom
    have hb := h_intN1.const_mul Cdom
    refine (ha.add hb).congr (Filter.Eventually.of_forall fun u => ?_)
    have h_pow : ‖u‖ * ‖u‖ ^ N = ‖u‖ ^ (N + 1) := by
      rw [show (N + 1 : ℕ) = 1 + N from by ring, pow_add, pow_one]
    show Cdom * (‖u‖ ^ 1 * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
          + Cdom * (‖u‖ ^ (N + 1) * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
        = Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
    rw [pow_one, ← h_pow]; ring
  refine h_int_dom.mono' h_int_cont.aestronglyMeasurable ?_
  filter_upwards with u
  rw [Real.norm_eq_abs]
  have h_norm_nn : 0 ≤ ‖u‖ := norm_nonneg _
  have h_gW_pos : 0 < gaussianWeight H u := gaussianWeight_pos H u
  ...
  have h_dot_b_le : |dot b u| ≤ bL1 * ‖u‖ := by
    rw [hbL1_def]; exact abs_dot_le_l1_mul_norm b u
  rw [abs_mul, abs_of_pos h_gW_pos]
  have h_B_t_le : |bulkErrA φ b hφ.toObservableTensorApprox t u|
      ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) := by
    unfold bulkErrA
    rw [show t * Real.sqrt t * dot b u *
            expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
        ...
        abs_mul, abs_of_pos (by positivity : 0 < t * Real.sqrt t), abs_mul]
    have h_t_nn : 0 ≤ t * Real.sqrt t := by positivity
    have h_dot_R := mul_le_mul h_dot_b_le (h_R_global u) (abs_nonneg _)
      (mul_nonneg hbL1_nn h_norm_nn)
    have h_step := mul_le_mul_of_nonneg_left h_dot_R h_t_nn
    have h_eq : t * Real.sqrt t * (bL1 * ‖u‖ * (R_const * (1 + ‖u‖ ^ N)))
        = t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) := by ring
  ...
  have h_first : |bulkErrA φ b hφ.toObservableTensorApprox t u| *
      ...
      ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
          gaussianWeight H u :=
    mul_le_mul_of_nonneg_right h_B_t_le h_gW_pos.le
  have h_step_nn : 0 ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) :=
    by positivity
  have h_second : t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
      gaussianWeight H u
      ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
          Real.exp (-((c / 2) * ‖u‖ ^ 2)) :=
    mul_le_mul_of_nonneg_left h_gW_le h_step_nn
  have h_eq_dom : t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
      Real.exp (-((c / 2) * ‖u‖ ^ 2))
      = Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2))) := by
    rw [hCdom_def]; ring
  linarith [h_first, h_second, h_eq_dom.le, h_eq_dom.ge]
...
private lemma bulkErrA_gaussian_asymptotic
  ...
  set δ : ℝ := hφ.toObservableTensorApprox.jet_radius with hδ_def
  have hδ_pos : 0 < δ := hφ.toObservableTensorApprox.jet_radius_pos
  have hδ2_pos : 0 < δ ^ 2 := pow_pos hδ_pos 2
  -- Tail asymptotic.
  obtain ⟨K_tail, M, hK_tail_nn, h_tail⟩ :=
    abs_bulkErrA_gaussian_symm_tail_le V φ H b hV hφ
  -- Local constant: bL1 · Q_const.
  set bL1 : ℝ := ∑ i, |b i| with hbL1_def
  have hbL1_nn : 0 ≤ bL1 := Finset.sum_nonneg (fun _ _ => abs_nonneg _)
  set K_loc : ℝ := bL1 * hφ.Q_const with hK_loc_def
  have hK_loc_nn : 0 ≤ K_loc := mul_nonneg hbL1_nn hφ.Q_const_nn
  -- Integrability moments.
  have h_int_loc6 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 2) 6
  have h_int_tail0 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 4) 0
  have h_int_tailM := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 4) M
  set I6 : ℝ := ∫ u : ι → ℝ, ‖u‖ ^ 6 *
    Real.exp (-((c / 2) * ‖u‖ ^ 2)) with hI6_def
  set I0 : ℝ := ∫ u : ι → ℝ, ‖u‖ ^ 0 *
    Real.exp (-((c / 4) * ‖u‖ ^ 2)) with hI0_def
  set IM : ℝ := ∫ u : ι → ℝ, ‖u‖ ^ M *
    Real.exp (-((c / 4) * ‖u‖ ^ 2)) with hIM_def
  have h_I6_nn : 0 ≤ I6 := by
    rw [hI6_def]; apply MeasureTheory.integral_nonneg; intro u; positivity
  have h_I0_nn : 0 ≤ I0 := by
    rw [hI0_def]; apply MeasureTheory.integral_nonneg; intro u; positivity
  have h_IM_nn : 0 ≤ IM := by
    rw [hIM_def]; apply MeasureTheory.integral_nonneg; intro u; positivity
  -- Final K = total / 2 (the symmetrization gives 2·∫B_t·gW).
  have hcδ2_pos : 0 < c * δ ^ 2 := mul_pos hc_pos hδ2_pos
  set Kbound : ℝ :=
    (K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / 2 with hKbound_def
  refine ⟨Kbound, 1, le_rfl, ?_⟩
  intro t ht_one
  have ht_pos : 0 < t := lt_of_lt_of_le zero_lt_one ht_one
  -- Pointwise majorants for the symmetric integrand `Sym`.
  set Hlocal : (ι → ℝ) → ℝ := fun u =>
    K_loc / t * ‖u‖ ^ 6 * Real.exp (-((c / 2) * ‖u‖ ^ 2)) with hHlocal_def
  set Htail : (ι → ℝ) → ℝ := fun u =>
    K_tail * (1 + ‖u‖ ^ M) * Real.exp (-((c / 4) * ‖u‖ ^ 2)) *
      Real.exp (-((c * δ ^ 2 / 4) * t)) with hHtail_def
  have hHlocal_nn : ∀ u, 0 ≤ Hlocal u := by
    intro u; rw [hHlocal_def]
    have h1 : 0 ≤ K_loc / t := div_nonneg hK_loc_nn ht_pos.le
    exact mul_nonneg (mul_nonneg h1 (pow_nonneg (norm_nonneg _) _))
  ...
  have h_pt : ∀ u : ι → ℝ,
    ...
    intro u
    by_cases hu : ‖u‖ ≤ δ * Real.sqrt t
    · have h := abs_bulkErrA_gaussian_symm_local_le V φ H b hV hφ ht_pos u hu
  ...
  -- Integrability of Hlocal.
  have hHlocal_int : Integrable Hlocal := by
    have h6 := h_int_loc6.const_mul (K_loc / t)
    refine h6.congr (Filter.Eventually.of_forall fun u => ?_)
    rw [hHlocal_def]; ring
  have hHtail_int : Integrable Htail := by
    have h0 := h_int_tail0.const_mul
      (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)))
    have hM := h_int_tailM.const_mul
      (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)))
    refine (h0.add hM).congr (Filter.Eventually.of_forall fun u => ?_)
    simp only [Pi.add_apply]
    show K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) *
            (‖u‖ ^ 0 * Real.exp (-((c / 4) * ‖u‖ ^ 2)))
          + K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) *
            (‖u‖ ^ M * Real.exp (-((c / 4) * ‖u‖ ^ 2)))
        = K_tail * (1 + ‖u‖ ^ M) * Real.exp (-((c / 4) * ‖u‖ ^ 2)) *
  ...
  have h_R_cont : Continuous (fun u : ι → ℝ =>
    ...
    have h_cubic_cont : Continuous (fun u : ι → ℝ =>
      ...
      continuous_const.mul (continuous_const.mul h_Φφ)
    exact ((h_phi_smul.sub h_lin_cont).sub h_quad_cont).sub h_cubic_cont
  have h_R_cont_neg : Continuous (fun u : ι → ℝ =>
      expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)) :=
    h_R_cont.comp continuous_neg
  have h_R_diff : Continuous (fun u : ι → ℝ =>
      expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
        - expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)) :=
    h_R_cont.sub h_R_cont_neg
  have h_Sym_cont : Continuous (fun u : ι → ℝ =>
      t * Real.sqrt t * dot b u *
        (expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
          - expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)) *
        gaussianWeight H u) :=
    ((continuous_const.mul h_dot_b).mul h_R_diff).mul (continuous_gaussianWeight H)
  -- Integrability of Sym via dominance.
  have h_int_Sym : Integrable (fun u : ι → ℝ =>
      t * Real.sqrt t * dot b u *
        (expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t u
          - expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t (-u)) *
        gaussianWeight H u) := by
    refine hHsum_int.mono' h_Sym_cont.aestronglyMeasurable ?_
    filter_upwards with u
    rw [Real.norm_eq_abs]
    exact h_pt u
  -- Apply symmetrization: 2·∫(B_t·gW) = ∫Sym.
  have hB_int : Integrable (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u) :=
    integrable_bulkErrA_mul_gaussianWeight V φ H b hV hφ ht_one
  have h_symm := bulkErrA_gaussian_symm φ H b hφ.toObservableTensorApprox ht_pos hB_int
  -- |∫Sym| ≤ ∫(Hloc + Htail).
  have h_abs_sym_le :
      |∫ u : ι → ℝ,
          t * Real.sqrt t * dot b u *
      ...
      ≤ ∫ u, Hlocal u + Htail u := by
    calc |∫ u : ι → ℝ, _|
        = ‖∫ u : ι → ℝ, _‖ := (Real.norm_eq_abs _).symm
      _ ≤ ∫ u, ‖_‖ := MeasureTheory.norm_integral_le_integral_norm _
      _ ≤ ∫ u, Hlocal u + Htail u := by
          apply MeasureTheory.integral_mono_ae h_int_Sym.norm hHsum_int
          filter_upwards with u
          rw [Real.norm_eq_abs]
          exact h_pt u
  -- Compute ∫Hloc.
  have h_Hlocal_int_eq : ∫ u, Hlocal u = K_loc / t * I6 := by
    calc ∫ u, Hlocal u
        = ∫ u, K_loc / t * (‖u‖ ^ 6 * Real.exp (-((c / 2) * ‖u‖ ^ 2))) := by
          apply MeasureTheory.integral_congr_ae
          filter_upwards with u
          rw [hHlocal_def]; ring
      _ = K_loc / t * I6 := by
          rw [MeasureTheory.integral_const_mul, ← hI6_def]
  -- Compute ∫Htail = K_tail · exp(-βt) · (I0 + IM).
  have h_Htail_int_eq : ∫ u, Htail u =
      K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) * (I0 + IM) := by
    have h0 := h_int_tail0.const_mul
      (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)))
    have hM := h_int_tailM.const_mul
      (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)))
    calc ∫ u, Htail u
        = ∫ u, (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t))) *
              (‖u‖ ^ 0 * Real.exp (-((c / 4) * ‖u‖ ^ 2)))
            + (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t))) *
              (‖u‖ ^ M * Real.exp (-((c / 4) * ‖u‖ ^ 2))) := by
          apply MeasureTheory.integral_congr_ae
          ...
          show K_tail * (1 + ‖u‖ ^ M) * Real.exp (-((c / 4) * ‖u‖ ^ 2)) *
              ...
              = K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) *
                ...
                + K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) *
                  (‖u‖ ^ M * Real.exp (-((c / 4) * ‖u‖ ^ 2)))
          rw [pow_zero]; ring
      _ = (∫ u, (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t))) *
              (‖u‖ ^ 0 * Real.exp (-((c / 4) * ‖u‖ ^ 2))))
          + ∫ u, (K_tail * Real.exp (-((c * δ ^ 2 / 4) * t))) *
              (‖u‖ ^ M * Real.exp (-((c / 4) * ‖u‖ ^ 2))) :=
          MeasureTheory.integral_add h0 hM
      _ = K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) * I0
          + K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) * IM := by
          rw [MeasureTheory.integral_const_mul,
              MeasureTheory.integral_const_mul, ← hI0_def, ← hIM_def]
      _ = K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) * (I0 + IM) := by ring
  have h_sum_eq : ∫ u, Hlocal u + Htail u = (∫ u, Hlocal u) + ∫ u, Htail u :=
    MeasureTheory.integral_add hHlocal_int hHtail_int
  -- exp(-βt) ≤ 1/(βt).
  have h_β_pos : 0 < c * δ ^ 2 / 4 := by positivity
  have h_exp_decay : Real.exp (-((c * δ ^ 2 / 4) * t)) ≤
      1 / ((c * δ ^ 2 / 4) * t) := by
    have h_pos : 0 < (c * δ ^ 2 / 4) * t := mul_pos h_β_pos ht_pos
    have hax := Real.add_one_le_exp ((c * δ ^ 2 / 4) * t)
    have h_le : (c * δ ^ 2 / 4) * t ≤ Real.exp ((c * δ ^ 2 / 4) * t) := by linarith
    rw [show Real.exp (-((c * δ ^ 2 / 4) * t)) = 1 / Real.exp ((c * δ ^ 2 / 4) * t)
        from by rw [Real.exp_neg]; ring]
    exact one_div_le_one_div_of_le h_pos h_le
  have h_tail_bd : ∫ u, Htail u
      ≤ 4 * K_tail / (c * δ ^ 2) * (I0 + IM) / t := by
    rw [h_Htail_int_eq]
    have h_factor_nn : 0 ≤ K_tail * (I0 + IM) := by positivity
    have h_β_inv_eq : 1 / ((c * δ ^ 2 / 4) * t) = 4 / (c * δ ^ 2 * t) := by
      field_simp
    rw [show K_tail * Real.exp (-((c * δ ^ 2 / 4) * t)) * (I0 + IM)
         = K_tail * (I0 + IM) * Real.exp (-((c * δ ^ 2 / 4) * t)) from by ring]
    have h_step := mul_le_mul_of_nonneg_left h_exp_decay h_factor_nn
    rw [h_β_inv_eq] at h_step
    have h_eq : K_tail * (I0 + IM) * (4 / (c * δ ^ 2 * t))
        = 4 * K_tail / (c * δ ^ 2) * (I0 + IM) / t := by field_simp
    linarith
  have h_combined : ∫ u, Hlocal u + Htail u ≤
      (K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / t := by
    rw [h_sum_eq, h_Hlocal_int_eq]
    rw [show (K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / t
        = K_loc * I6 / t + 4 * K_tail / (c * δ ^ 2) * (I0 + IM) / t from by
      field_simp]
    have h_K_loc_eq : K_loc / t * I6 = K_loc * I6 / t := by ring
    linarith
  -- Combine: |2·∫B_t·gW| = |∫Sym| ≤ K_total / t.
  have h_two_combined :
      |2 * (∫ u : ι → ℝ, bulkErrA φ b hφ.toObservableTensorApprox t u *
          gaussianWeight H u)|
      ≤ (K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / t := by
    rw [h_symm]
    exact le_trans h_abs_sym_le h_combined
  -- Strip the factor of 2: |∫B_t·gW| ≤ Kbound / t.
  have h_two_abs :
      |2 * (∫ u : ι → ℝ, bulkErrA φ b hφ.toObservableTensorApprox t u *
          gaussianWeight H u)|
      = 2 * |∫ u : ι → ℝ, bulkErrA φ b hφ.toObservableTensorApprox t u *
          gaussianWeight H u| := by
    rw [abs_mul, abs_of_pos (by norm_num : (0 : ℝ) < 2)]
  rw [h_two_abs] at h_two_combined
  have h_eq : Kbound / t =
      (K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / (2 * t) := by
    rw [hKbound_def]; field_simp
  rw [h_eq]
  have h_t_ne : t ≠ 0 := ne_of_gt ht_pos
  have h_div_eq :
      (K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / t
      = 2 * ((K_loc * I6 + 4 * K_tail / (c * δ ^ 2) * (I0 + IM)) / (2 * t)) := by
    field_simp
  rw [h_div_eq] at h_two_combined
  linarith

/-- **`bulkErrA · gW · exp(-s_t)` integrability** (helper for main theorem
`rescaledIntegral_cross_linear_connected_asymptotic`).

Polynomial dominance:
- `|bulkErrA(u)| ≤ t·√t·‖b‖_1·‖u‖·R_const·(1+‖u‖^N)` globally for `t ≥ 1`
  via `abs_expNumObsRem_global_le`.
- `gW · exp(-s_t) ≤ exp(-c·‖u‖²)` via `rescaled_weight_le_coercive`.
- Combined: dominator `Cdom · ‖u‖·(1+‖u‖^N) · exp(-c·‖u‖²)` is integrable.

Same dominance pattern as in `abs_integral_bulkErrA_le`; extracted here as
a standalone helper for clean use in the main theorem assembly. -/
private lemma integrable_bulkErrA_mul_rescaled_weight
    (V φ : (ι → ℝ) → ℝ)
    (H : (ι → ℝ) →L[ℝ] (ι → ℝ))
    (b : ι → ℝ)
    [Nonempty ι]
  ...
  have hVc : Continuous V := hV.toPotentialApprox.V_continuous
  have hφ_cont : Continuous φ := hφ.toObservableApprox.phi_continuous
  -- Setup constants for global polynomial bound on expNumObsRem.
  obtain ⟨Kφ, p, hKφ_nn, h_poly_φ⟩ := hφ.toObservableApprox.poly_growth
  set N : ℕ := max p 3 with hN_def
  set N₁ : ℝ := (Fintype.card ι : ℝ) with hN1_def
  have hN1_nn : 0 ≤ N₁ := by rw [hN1_def]; exact_mod_cast Nat.zero_le _
  ...
  set bL1 : ℝ := ∑ i, |b i| with hbL1_def
  have hbL1_nn : 0 ≤ bL1 := Finset.sum_nonneg (fun _ _ => abs_nonneg _)
  -- |expNumObsRem(u)| ≤ R_const · (1+‖u‖^N) globally for t ≥ 1.
  have h_R_global : ∀ v : ι → ℝ,
      |expNumObsRem φ (0 : ι → ℝ) hφ.toObservableTensorApprox t v|
        ≤ R_const * (1 + ‖v‖ ^ N) := by
    intro v
  ...
  have h_bulkErrA_cont : Continuous (fun u : ι → ℝ =>
    ...
    unfold bulkErrA
    exact (continuous_const.mul h_dot_b).mul h_R_cont
  have h_exp_cont : Continuous (fun u : ι → ℝ =>
      Real.exp (-(rescaledPerturbation V H t u))) :=
    Real.continuous_exp.comp (continuous_rescaledPerturbation hVc H t).neg
  have h_int_cont : Continuous (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u *
        Real.exp (-(rescaledPerturbation V H t u))) :=
    (h_bulkErrA_cont.mul (continuous_gaussianWeight H)).mul h_exp_cont
  -- Polynomial dominator.
  set Cdom : ℝ := t * Real.sqrt t * bL1 * R_const with hCdom_def
  have hCdom_nn : 0 ≤ Cdom := by rw [hCdom_def]; positivity
  have h_int1 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι) hc_pos 1
  have h_intN1 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι) hc_pos (N + 1)
  have h_int_dom : Integrable (fun u : ι → ℝ =>
      Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-(c * ‖u‖ ^ 2)))) := by
    have ha := h_int1.const_mul Cdom
    have hb := h_intN1.const_mul Cdom
    refine (ha.add hb).congr (Filter.Eventually.of_forall fun u => ?_)
    have h_pow : ‖u‖ * ‖u‖ ^ N = ‖u‖ ^ (N + 1) := by
      rw [show (N + 1 : ℕ) = 1 + N from by ring, pow_add, pow_one]
    show Cdom * (‖u‖ ^ 1 * Real.exp (-(c * ‖u‖ ^ 2)))
          + Cdom * (‖u‖ ^ (N + 1) * Real.exp (-(c * ‖u‖ ^ 2)))
        = Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-(c * ‖u‖ ^ 2)))
    rw [pow_one, ← h_pow]; ring
  ...
  have h_combined_pos : 0 < gaussianWeight H u *
      Real.exp (-(rescaledPerturbation V H t u)) :=
    mul_pos h_gW_pos (Real.exp_pos _)
  ...
  rw [show bulkErrA φ b hφ.toObservableTensorApprox t u *
        ...
        = bulkErrA φ b hφ.toObservableTensorApprox t u *
          (gaussianWeight H u * Real.exp (-(rescaledPerturbation V H t u)))
      from by ring,
      abs_mul, abs_of_pos h_combined_pos]
  have h_B_t_le : |bulkErrA φ b hφ.toObservableTensorApprox t u|
      ≤ Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) := by
    unfold bulkErrA
    rw [show t * Real.sqrt t * dot b u *
    ...
    have h_step := mul_le_mul_of_nonneg_left h_dot_R h_t_nn
    have h_eq : t * Real.sqrt t * (bL1 * ‖u‖ * (R_const * (1 + ‖u‖ ^ N)))
        = Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) := by rw [hCdom_def]; ring
    linarith
  have h_first : |bulkErrA φ b hφ.toObservableTensorApprox t u| *
      (gaussianWeight H u * Real.exp (-(rescaledPerturbation V H t u)))
      ≤ Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) *
          (gaussianWeight H u * Real.exp (-(rescaledPerturbation V H t u))) :=
    mul_le_mul_of_nonneg_right h_B_t_le h_combined_pos.le
  have h_step_nn : 0 ≤ Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) := by
    apply mul_nonneg hCdom_nn (by positivity)
  have h_second : Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) *
      (gaussianWeight H u * Real.exp (-(rescaledPerturbation V H t u)))
      ≤ Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) * Real.exp (-(c * ‖u‖ ^ 2)) :=
    mul_le_mul_of_nonneg_left h_combined_le h_step_nn
  have h_eq_dom : Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) * Real.exp (-(c * ‖u‖ ^ 2))
      = Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-(c * ‖u‖ ^ 2))) := by ring
  linarith [h_first, h_second, h_eq_dom.le, h_eq_dom.ge]
...
`|∫ bulkErrA · gW · exp(-s_t)| ≤ K/t` by triangle inequality on the
`exp(-s_t) = 1 + (exp(-s_t) - 1)` split, combining the asymptotics
`bulkErrA_gaussian_asymptotic` (Gaussian piece) and
`bulkErrA_exp_sub_one_asymptotic` (perturbative piece). -/
private lemma abs_integral_bulkErrA_le
    ...
    ∃ K T₀ : ℝ, 1 ≤ T₀ ∧ ∀ t : ℝ, T₀ ≤ t →
      |∫ u : ι → ℝ,
        ...
        ≤ K / t := by
  classical
  -- Apply the two asymptotics from the new architecture (per GPT alt-path).
  obtain ⟨K1, T1, hT1, h1⟩ := bulkErrA_gaussian_asymptotic V φ H b hV hφ
  obtain ⟨K2, T2, hT2, h2⟩ := bulkErrA_exp_sub_one_asymptotic V φ H b hV hφ
  refine ⟨K1 + K2, max T1 T2, le_max_of_le_left hT1, ?_⟩
  intro t ht
  have ht1 : T1 ≤ t := le_of_max_le_left ht
  have ht2 : T2 ≤ t := le_of_max_le_right ht
  have ht_one : 1 ≤ t := le_trans hT1 ht1
  have ht_pos : 0 < t := lt_of_lt_of_le zero_lt_one ht_one
  have hsqrt_pos : 0 < Real.sqrt t := Real.sqrt_pos.mpr ht_pos
  have hB1 := h1 t ht1
  have hB2 := h2 t ht2
  -- Setup for integrability.
  set c : ℝ := hV.toPotentialApprox.coercive_const with hc_def
  have hc_pos : 0 < c := hV.toPotentialApprox.coercive_const_pos
  have h_coer := hV.toPotentialApprox.coercive_bound
  have hVc : Continuous V := hV.toPotentialApprox.V_continuous
  ...
  have h_bulkErrA_cont : Continuous (fun u : ι → ℝ =>
    ...
    unfold bulkErrA
    exact (continuous_const.mul h_dot_b).mul h_R_cont
  have h_BgW_cont : Continuous (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u) :=
    h_bulkErrA_cont.mul (continuous_gaussianWeight H)
  have h_exp_cont : Continuous (fun u : ι → ℝ =>
      Real.exp (-(rescaledPerturbation V H t u))) :=
    Real.continuous_exp.comp (continuous_rescaledPerturbation hVc H t).neg
  have h_exp_sub_one_cont : Continuous (fun u : ι → ℝ =>
      Real.exp (-(rescaledPerturbation V H t u)) - 1) :=
    h_exp_cont.sub continuous_const
  have h_LHS_cont : Continuous (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u *
        Real.exp (-(rescaledPerturbation V H t u))) :=
    h_BgW_cont.mul h_exp_cont
  have h_pert_cont : Continuous (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u *
        (Real.exp (-(rescaledPerturbation V H t u)) - 1)) :=
    h_BgW_cont.mul h_exp_sub_one_cont
  -- Polynomial dominator: |B_t(u)·gW(u)·X(u)| ≤ t·√t·bL1·R_const ·
  -- ‖u‖·(1+‖u‖^N) · exp(-c·‖u‖²) where X is `exp(-s_t)` (using rescaled_weight).
  -- For B_t · gW alone, X = 1 effectively, with `gW ≤ exp(-c/2·‖u‖²)`.
  -- For B_t · gW · (exp - 1), use uniform: |gW · (exp-1)| ≤ gW + exp(-c·‖u‖²),
  -- so dominator is (t·√t·bL1·R_const · ‖u‖·(1+‖u‖^N)) · 2·exp(-c/2·‖u‖²).
  set Cdom : ℝ := t * Real.sqrt t * bL1 * R_const with hCdom_def
  have hCdom_nn : 0 ≤ Cdom := by rw [hCdom_def]; positivity
  -- Dominator integrability: ∫ Cdom · ‖u‖·(1+‖u‖^N) · exp(-c/2·‖u‖²) du < ∞.
  have h_int1 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 2) 1
  have h_intN1 := integrable_norm_pow_mul_exp_neg_const_sq (ι := ι)
    (by linarith : (0 : ℝ) < c / 2) (N + 1)
  have h_int_dom_BgW : Integrable (fun u : ι → ℝ =>
      Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2)))) := by
    have ha := h_int1.const_mul Cdom
    have hb := h_intN1.const_mul Cdom
    refine (ha.add hb).congr (Filter.Eventually.of_forall fun u => ?_)
    ...
    show Cdom * (‖u‖ ^ 1 * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
        ...
        = Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
    rw [pow_one, ← h_pow]; ring
  -- Pointwise bound on |B_t · gW|: ≤ Cdom · ‖u‖·(1+‖u‖^N) · exp(-c/2·‖u‖²).
  have h_BgW_dom : ∀ u, |bulkErrA φ b hφ.toObservableTensorApprox t u *
      gaussianWeight H u| ≤ Cdom *
        (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2))) := by
    intro u
    have h_norm_nn : 0 ≤ ‖u‖ := norm_nonneg _
    have h_gW_pos : 0 < gaussianWeight H u := gaussianWeight_pos H u
    have h_gW_le : gaussianWeight H u ≤ Real.exp (-((c / 2) * ‖u‖ ^ 2)) :=
    ...
    have h_first : |bulkErrA φ b hφ.toObservableTensorApprox t u| *
        ...
        ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
            gaussianWeight H u :=
      mul_le_mul_of_nonneg_right h_B_t_le h_gW_pos.le
    have h_step_nn : 0 ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) := by
      positivity
    have h_second : t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
        gaussianWeight H u
        ≤ t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
    ...
    have h_eq_dom : t * Real.sqrt t * (bL1 * ‖u‖) * (R_const * (1 + ‖u‖ ^ N)) *
      ...
      rw [hCdom_def]; ring
    linarith [h_first, h_second, h_eq_dom.le, h_eq_dom.ge]
  -- Integrability of B_t · gW.
  have h_int_BgW : Integrable (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u) := by
    refine h_int_dom_BgW.mono' h_BgW_cont.aestronglyMeasurable ?_
    filter_upwards with u
    rw [Real.norm_eq_abs]
    exact h_BgW_dom u
  -- For B_t · gW · (exp - 1), use uniform bound:
  --   |gW · (exp - 1)| ≤ gW + exp(-c·‖u‖²) ≤ 2 · exp(-(c/2)·‖u‖²).
  -- Wait: gW ≤ exp(-(c/2)·‖u‖²) AND exp(-c·‖u‖²) ≤ exp(-(c/2)·‖u‖²) (since c > c/2).
  -- So |gW · (exp - 1)| ≤ 2 · exp(-(c/2)·‖u‖²).
  -- Therefore |B_t · gW · (exp - 1)| ≤ |B_t| · 2 · exp(-(c/2)·‖u‖²).
  have h_int_dom_pert : Integrable (fun u : ι → ℝ =>
      (2 * Cdom) * (‖u‖ * (1 + ‖u‖ ^ N) *
        Real.exp (-((c / 2) * ‖u‖ ^ 2)))) := by
    refine h_int_dom_BgW.const_mul 2 |>.congr (Filter.Eventually.of_forall ?_)
    intro u
    show 2 * (Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2))))
        = 2 * Cdom * (‖u‖ * (1 + ‖u‖ ^ N) * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
    ring
  have h_pert_dom : ∀ u, |bulkErrA φ b hφ.toObservableTensorApprox t u *
      gaussianWeight H u *
      (Real.exp (-(rescaledPerturbation V H t u)) - 1)|
      ≤ (2 * Cdom) * (‖u‖ * (1 + ‖u‖ ^ N) *
          Real.exp (-((c / 2) * ‖u‖ ^ 2))) := by
    ...
    have h_norm_nn : 0 ≤ ‖u‖ := norm_nonneg _
    have h_gW_pos : 0 < gaussianWeight H u := gaussianWeight_pos H u
    -- |B_t · gW · (exp - 1)| ≤ |B_t| · |gW · (exp - 1)|
    have h_eq_split : bulkErrA φ b hφ.toObservableTensorApprox t u *
        gaussianWeight H u *
        (Real.exp (-(rescaledPerturbation V H t u)) - 1)
        = bulkErrA φ b hφ.toObservableTensorApprox t u *
            (gaussianWeight H u *
              (Real.exp (-(rescaledPerturbation V H t u)) - 1)) := by ring
    rw [h_eq_split, abs_mul]
    -- Use uniform bound: |gW · (exp - 1)| ≤ gW + exp(-c·‖u‖²).
    have h_uniform := abs_gaussianWeight_mul_exp_sub_one_le_uniform V H hc_pos
      h_coer ht_pos u
    -- Combined: gW + exp(-c·‖u‖²) ≤ 2 · exp(-(c/2)·‖u‖²).
    have h_gW_le : gaussianWeight H u ≤ Real.exp (-((c / 2) * ‖u‖ ^ 2)) :=
      gaussianWeight_le_exp_neg_coercive V H hV.toPotentialTensorApprox u
    have h_exp_le : Real.exp (-(c * ‖u‖ ^ 2))
        ≤ Real.exp (-((c / 2) * ‖u‖ ^ 2)) := by
      ...
      nlinarith
    have h_sum_le : gaussianWeight H u + Real.exp (-(c * ‖u‖ ^ 2))
        ≤ 2 * Real.exp (-((c / 2) * ‖u‖ ^ 2)) := by linarith
    have h_pert_le : |gaussianWeight H u *
        (Real.exp (-(rescaledPerturbation V H t u)) - 1)|
        ≤ 2 * Real.exp (-((c / 2) * ‖u‖ ^ 2)) := le_trans h_uniform h_sum_le
    -- |B_t| ≤ Cdom · ‖u‖·(1+‖u‖^N) (extract from h_BgW_dom).
    have h_dot_b : |dot b u| ≤ bL1 * ‖u‖ := by
      rw [hbL1_def]; exact abs_dot_le_l1_mul_norm b u
    have h_B_t_le : |bulkErrA φ b hφ.toObservableTensorApprox t u|
        ≤ Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) := by
      ...
      have h_eq : t * Real.sqrt t * (bL1 * ‖u‖ * (R_const * (1 + ‖u‖ ^ N)))
          = Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) := by rw [hCdom_def]; ring
      linarith
    -- Combine: |B_t|·|gW·(exp-1)| ≤ Cdom·‖u‖·(1+‖u‖^N)·2·exp(-c/2·‖u‖²)
    --        = 2·Cdom·‖u‖·(1+‖u‖^N)·exp(-c/2·‖u‖²).
    have h_step1 := mul_le_mul h_B_t_le h_pert_le (abs_nonneg _)
      (mul_nonneg hCdom_nn (by positivity))
    -- h_step1 : |B_t|·|gW·(exp-1)| ≤ Cdom·(‖u‖·(1+‖u‖^N))·(2·exp(-c/2·‖u‖²))
    have h_eq_final : Cdom * (‖u‖ * (1 + ‖u‖ ^ N)) *
        (2 * Real.exp (-((c / 2) * ‖u‖ ^ 2)))
        = 2 * Cdom * (‖u‖ * (1 + ‖u‖ ^ N) *
            Real.exp (-((c / 2) * ‖u‖ ^ 2))) := by ring
    linarith [h_step1, h_eq_final.le, h_eq_final.ge]
  -- Integrability of B_t · gW · (exp - 1).
  have h_int_pert : Integrable (fun u : ι → ℝ =>
      bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u *
        (Real.exp (-(rescaledPerturbation V H t u)) - 1)) := by
    refine h_int_dom_pert.mono' h_pert_cont.aestronglyMeasurable ?_
    filter_upwards with u
    rw [Real.norm_eq_abs]
    exact h_pert_dom u
  -- Integral identity: ∫ B_t · gW · exp(-s_t) = ∫ B_t · gW + ∫ B_t · gW · (exp - 1).
  have h_int_id :
      ∫ u : ι → ℝ, bulkErrA φ b hφ.toObservableTensorApprox t u *
          gaussianWeight H u *
          Real.exp (-(rescaledPerturbation V H t u))
        ...
        bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u *
          ...
          = bulkErrA φ b hφ.toObservableTensorApprox t u * gaussianWeight H u
            + bulkErrA φ b hφ.toObservableTensorApprox t u *
                ...
                (Real.exp (-(rescaledPerturbation V H t u)) - 1) := by
      intro u; ring
    calc ∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
            gaussianWeight H u *
            Real.exp (-(rescaledPerturbation V H t u))
        = ∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
              gaussianWeight H u
            + bulkErrA φ b hφ.toObservableTensorApprox t u *
                ...
                (Real.exp (-(rescaledPerturbation V H t u)) - 1) := by
          apply MeasureTheory.integral_congr_ae
          filter_upwards with u; exact h_pt u
      _ = (∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
              gaussianWeight H u)
          + ∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
              gaussianWeight H u *
              (Real.exp (-(rescaledPerturbation V H t u)) - 1) :=
          MeasureTheory.integral_add h_int_BgW h_int_pert
  -- Triangle.
  rw [h_int_id]
  calc |(∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
              gaussianWeight H u)
      ...
      ≤ |∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
        ...
        + |∫ u, bulkErrA φ b hφ.toObservableTensorApprox t u *
              gaussianWeight H u *
              (Real.exp (-(rescaledPerturbation V H t u)) - 1)| := abs_add_le _ _
    _ ≤ K1 / t + K2 / t := add_le_add hB1 hB2
    _ = (K1 + K2) / t := by ring
