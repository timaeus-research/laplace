...
/-- The limiting domain of the rescaling `x = t^{-α} u` on the box of radius `ρ`. -/
def limitDomain (ρ D γ q : ℝ) (Q α : ι → ℝ) : Set (ι → ℝ) :=
  Set.pi univ (fun j ↦ if α j = 0 then Ioo 0 ρ else Ioi 0) ∩
    {u | ∑ j, Q j * α j = γ → D * ∏ j, u j ^ (-(Q j / q)) < ρ}

variable {ρ D γ q : ℝ} {Q α : ι → ℝ}

theorem limitDomain_pos {u : ι → ℝ} (hu : u ∈ limitDomain ρ D γ q Q α) (j : ι) : 0 < u j := by
...
theorem rescaledDomain_subset {t : ℝ} :
    rescaledDomain ρ D γ q Q α t ⊆ limitDomain ρ D γ q Q α := by
  intro u hu
  obtain ⟨h1, h2⟩ := hu
  refine ⟨Set.mem_univ_pi.mpr fun j ↦ ?_, fun htied ↦ ?_⟩
  · have hj := Set.mem_univ_pi.mp h1 j
    split_ifs with h0
    · rwa [h0, Real.rpow_zero, mul_one] at hj
    · exact hj.1
  · have h2' : rescaledCut D γ q Q α t u < ρ := h2
    unfold rescaledCut at h2'
    rwa [htied, sub_self, zero_div, neg_zero, Real.rpow_zero, mul_one] at h2'

theorem eventually_mem_rescaledDomain (hρ : 0 < ρ) (hq : 0 < q) (hα : ∀ j, 0 ≤ α j)
    (htruth : ∑ j, Q j * α j ≤ γ) {u : ι → ℝ} (hu : u ∈ limitDomain ρ D γ q Q α) :
    ∀ᶠ t in atTop, u ∈ rescaledDomain ρ D γ q Q α t := by
  have hpi : ∀ᶠ t in atTop, ∀ j, u j ∈ Ioo 0 (ρ * t ^ α j) := by
    refine Filter.eventually_all.2 fun j ↦ ?_
    have hj := Set.mem_univ_pi.mp hu.1 j
    by_cases h0 : α j = 0
    · rw [if_pos h0] at hj
      exact Eventually.of_forall fun t ↦ by rw [h0, Real.rpow_zero, mul_one]; exact hj
    · rw [if_neg h0] at hj
      have hpos : 0 < α j := lt_of_le_of_ne (hα j) (Ne.symm h0)
      filter_upwards [((tendsto_rpow_atTop hpos).const_mul_atTop hρ).eventually_gt_atTop (u j)]
        with t ht
      exact ⟨hj, ht⟩
  have hcut : ∀ᶠ t in atTop, rescaledCut D γ q Q α t u < ρ := by
...
theorem measurableSet_limitDomain : MeasurableSet (limitDomain ρ D γ q Q α) := by
  refine (MeasurableSet.pi countable_univ fun j _ ↦ ?_).inter ?_
  · split_ifs
    · exact measurableSet_Ioo
    · exact measurableSet_Ioi
  · by_cases htied : ∑ j, Q j * α j = γ
    · have : {u : ι → ℝ | ∑ j, Q j * α j = γ → D * ∏ j, u j ^ (-(Q j / q)) < ρ} =
    ...
    · have : {u : ι → ℝ | ∑ j, Q j * α j = γ → D * ∏ j, u j ^ (-(Q j / q)) < ρ} = univ := by
        ext u; simp [htied]
      rw [this]
...
theorem measurableSet_rescaledDomain (t : ℝ) :
    MeasurableSet (rescaledDomain ρ D γ q Q α t) :=
...
`t < 1`. -/
noncomputable def dsPhase (ρ B D γ q δ : ℝ) (Q κ α : ι → ℝ) (a : (ι → ℝ) → ℝ → ℝ) (t : ℝ)
    (u : ι → ℝ) : ℝ :=
  if 1 ≤ t ∧ u ∈ limitDomain ρ D γ q Q α then
...
noncomputable def dsWeight (ρ D γ q : ℝ) (Q r α : ι → ℝ) (W : (ι → ℝ) → ℝ → ℝ) (t : ℝ)
    (u : ι → ℝ) : ℝ :=
  if 1 ≤ t then
    (rescaledDomain ρ D γ q Q α t).indicator
...
`0` otherwise. -/
noncomputable def dsProfile (ρ B D γ q δ : ℝ) (Q κ α : ι → ℝ) (a₀ : (ι → ℝ) → ℝ) (u : ι → ℝ) :
    ℝ :=
  if u ∈ limitDomain ρ D γ q Q α then
    (if ∑ j, κ j * α j = δ then B * a₀ u * ∏ j, u j ^ κ j else 0)
  else 0

/-- The limiting weight `1_{limitDomain} W₀ ∏ u^r`. -/
noncomputable def dsWeight₀ (ρ D γ q : ℝ) (Q r α : ι → ℝ) (W₀ : (ι → ℝ) → ℝ) (u : ι → ℝ) : ℝ :=
  (limitDomain ρ D γ q Q α).indicator (fun u ↦ W₀ u * ∏ j, u j ^ r j) u

/-- The envelope `Wmax 1_{limitDomain} ∏ u^r`. -/
noncomputable def dsEnvelope (ρ D γ q : ℝ) (Q r α : ι → ℝ) (Wmax : ℝ) (u : ι → ℝ) : ℝ :=
  Wmax * (limitDomain ρ D γ q Q α).indicator (fun u ↦ ∏ j, u j ^ r j) u
...
theorem dsWeight_mul_exp {t : ℝ} (ht : 1 ≤ t) (u : ι → ℝ) :
    dsWeight ρ D γ q Q r α W t u * exp (-(dsPhase ρ B D γ q δ Q κ α a t u)) =
      rescaledIntegrand ρ B D γ q δ Q κ r α W a t u := by
  unfold dsWeight dsPhase rescaledIntegrand
  rw [if_pos ht]
  by_cases hu : u ∈ rescaledDomain ρ D γ q Q α t
...
theorem dsWeight_ne_zero {t : ℝ} {u : ι → ℝ} (hw : dsWeight ρ D γ q Q r α W t u ≠ 0) :
    1 ≤ t ∧ u ∈ rescaledDomain ρ D γ q Q α t ∧
...
structure DominantScaleHyp (ρ B D γ q δ : ℝ) (Q κ r α : ι → ℝ) (W a : (ι → ℝ) → ℝ → ℝ)
  ...
  a₀_bounds : ∀ u ∈ limitDomain ρ D γ q Q α, amin ≤ a₀ u ∧ a₀ u ≤ amax
  W_lim : ∀ u ∈ limitDomain ρ D γ q Q α,
    Tendsto (fun t ↦ W (rescale t α u) (rescaledCut D γ q Q α t u)) atTop (𝓝 (W₀ u))
  a_lim : ∀ u ∈ limitDomain ρ D γ q Q α,
    Tendsto (fun t ↦ a (rescale t α u) (rescaledCut D γ q Q α t u)) atTop (𝓝 (a₀ u))
  int : Integrable (fun u ↦ dsEnvelope ρ D γ q Q r α Wmax u *
    exp (-(amin / amax * dsProfile ρ B D γ q δ Q κ α a₀ u)))
  Φint : Integrable (fun u ↦ dsEnvelope ρ D γ q Q r α Wmax u *
    (dsProfile ρ B D γ q δ Q κ α a₀ u * exp (-(amin / amax * dsProfile ρ B D γ q δ Q κ α a₀ u))))

namespace DominantScaleHyp

variable {amin amax : ℝ} (h : DominantScaleHyp ρ B D γ q δ Q κ r α W a W₀ a₀ Wmax amin amax)
include h
...
theorem measurable_dsPhase (ha : Measurable (Function.uncurry a)) (t : ℝ) :
    Measurable (dsPhase ρ B D γ q δ Q κ α a t) := by
  classical
  by_cases ht : 1 ≤ t
  · have e : dsPhase ρ B D γ q δ Q κ α a t = fun u ↦ if u ∈ limitDomain ρ D γ q Q α then
  ...
  · have e : dsPhase ρ B D γ q δ Q κ α a t = fun _ ↦ 0 := by
...
theorem measurable_dsWeight (hW : Measurable (Function.uncurry W)) (t : ℝ) :
    Measurable (dsWeight ρ D γ q Q r α W t) := by
  by_cases ht : 1 ≤ t
  · have e : dsWeight ρ D γ q Q r α W t = (rescaledDomain ρ D γ q Q α t).indicator
  ...
  · have e : dsWeight ρ D γ q Q r α W t = fun _ ↦ 0 := by
      funext u; simp only [dsWeight, ht, if_false]
    rw [e]
    exact measurable_const

theorem dsProfile_nonneg (u : ι → ℝ) : 0 ≤ dsProfile ρ B D γ q δ Q κ α a₀ u := by
  unfold dsProfile
...
theorem dsPhase_ge {t : ℝ} {u : ι → ℝ} (hw : dsWeight ρ D γ q Q r α W t u ≠ 0) :
    B * t ^ (δ - ∑ j, κ j * α j) * amin * ∏ j, u j ^ κ j ≤ dsPhase ρ B D γ q δ Q κ α a t u ∧
      0 ≤ dsPhase ρ B D γ q δ Q κ α a t u := by
...
theorem dsPhase_tendsto (u : ι → ℝ) :
    Tendsto (fun t ↦ dsPhase ρ B D γ q δ Q κ α a t u) atTop
      (𝓝 (dsProfile ρ B D γ q δ Q κ α a₀ u)) := by
  by_cases hu : u ∈ limitDomain ρ D γ q Q α
  · have hev : (fun t ↦ dsPhase ρ B D γ q δ Q κ α a t u) =ᶠ[atTop] fun t ↦
        B * t ^ (δ - ∑ j, κ j * α j) * a (rescale t α u) (rescaledCut D γ q Q α t u) *
          ∏ j, u j ^ κ j := by
  ...
  · have e : (fun t ↦ dsPhase ρ B D γ q δ Q κ α a t u) = fun _ ↦ 0 := by
...
theorem dsWeight_tendsto (u : ι → ℝ) :
    Tendsto (fun t ↦ dsWeight ρ D γ q Q r α W t u) atTop (𝓝 (dsWeight₀ ρ D γ q Q r α W₀ u)) := by
  by_cases hu : u ∈ limitDomain ρ D γ q Q α
  · have hev : (fun t ↦ dsWeight ρ D γ q Q r α W t u) =ᶠ[atTop] fun t ↦
        W (rescale t α u) (rescaledCut D γ q Q α t u) * ∏ j, u j ^ r j := by
      filter_upwards [eventually_ge_atTop 1,
        eventually_mem_rescaledDomain h.hρ h.hq h.feasible.1 h.feasible.2.1 hu] with t ht hmem
      unfold dsWeight
      rw [if_pos ht, Set.indicator_of_mem hmem]
    unfold dsWeight₀
    rw [Set.indicator_of_mem hu]
    exact ((h.W_lim u hu).mul_const _).congr' hev.symm
  · have e : (fun t ↦ dsWeight ρ D γ q Q r α W t u) = fun _ ↦ 0 := by
      funext t
      by_contra hne
...
theorem abs_dsWeight_le (t : ℝ) (u : ι → ℝ) :
    |dsWeight ρ D γ q Q r α W t u| ≤ dsEnvelope ρ D γ q Q r α Wmax u := by
  have henv : 0 ≤ dsEnvelope ρ D γ q Q r α Wmax u :=
    mul_nonneg h.Wmax_nonneg (Set.indicator_nonneg (fun u hu ↦
      Finset.prod_nonneg fun j _ ↦ rpow_nonneg (limitDomain_pos hu j).le _) u)
  by_cases hw : dsWeight ρ D γ q Q r α W t u = 0
...
theorem lower {t : ℝ} {u : ι → ℝ} (hw : dsWeight ρ D γ q Q r α W t u ≠ 0) :
    amin / amax * dsProfile ρ B D γ q δ Q κ α a₀ u ≤ dsPhase ρ B D γ q δ Q κ α a t u := by
  ...
  · rw [if_pos htied]
    ...
    calc amin / amax * (B * a₀ u * ∏ j, u j ^ κ j)
      ...
      _ ≤ dsPhase ρ B D γ q δ Q κ α a t u := hge
  · rw [if_neg htied, mul_zero]
    exact hnn

/-- **The dominant-scale certificate**: the moving data form a `RescaledData`. -/
theorem rescaledData :
    RescaledData atTop volume (dsPhase ρ B D γ q δ Q κ α a) (dsWeight ρ D γ q Q r α W)
      (dsProfile ρ B D γ q δ Q κ α a₀) (dsWeight₀ ρ D γ q Q r α W₀) (dsEnvelope ρ D γ q Q r α Wmax)
      (amin / amax) where
...
theorem tendsto_modelKernel (A p : ℝ) :
    Tendsto (fun t ↦ t ^ lpExponent γ p (fun j ↦ r j + 1) α *
      ...
      (𝓝 (A * ∫ u, dsWeight₀ ρ D γ q Q r α W₀ u * exp (-dsProfile ρ B D γ q δ Q κ α a₀ u))) := by
  ...
  filter_upwards [eventually_ge_atTop 1] with t ht
  have ht0 : 0 < t := zero_lt_one.trans_le ht
  rw [modelKernel_eq_rescaled (ρ := ρ) (α := α) A p h.W_meas h.a_meas ht0]
  have hI : ∫ u, rescaledIntegrand ρ B D γ q δ Q κ r α W a t u =
      ∫ u, dsWeight ρ D γ q Q r α W t u * exp (-(dsPhase ρ B D γ q δ Q κ α a t u)) :=
