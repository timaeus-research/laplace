...
def logCut (ρ D γ q t : ℝ) (Q : Fin n → ℝ) : Set (Fin n → ℝ) :=
  {z | ∑ i, Q i * z i < q * log (ρ / D) + γ * log t + (∑ i, Q i) * log ρ}

/-- The cutoff variable at `ρ e^{-z}`. -/
theorem cutVar_negExp {ρ D γ q t : ℝ} (hρ : 0 < ρ) (Q : Fin n → ℝ) (z : Fin n → ℝ) :
...
theorem modelIntegrand_const_negExp {ρ B D γ q δ : ℝ} {Q κ r : Fin n → ℝ} {w₀ a₀ t : ℝ}
    (hρ : 0 < ρ) (hD : 0 < D) (hq : 0 < q) (ht : 0 < t) {z : Fin n → ℝ} (hz : ∀ i, 0 < z i) :
    (∏ i, ρ * exp (-z i)) *
  ...
  have hbox : negExpMap ρ z ∈ Set.pi univ fun _ : Fin n ↦ Ioo (0 : ℝ) ρ := by
  ...
  · rw [Set.indicator_of_mem hcut, Set.indicator_of_mem
      (show negExpMap ρ z ∈ (Set.pi univ fun _ : Fin n ↦ Ioo (0 : ℝ) ρ) ∩
    ...
    have eκ : ∏ i, (negExpMap ρ z) i ^ κ i = ρ ^ (∑ i, κ i) * exp (-(∑ i, κ i * z i)) := by
      ...
      rw [Finset.prod_mul_distrib, ← Real.exp_sum, ← Real.rpow_sum_of_pos hρ,
        Finset.sum_neg_distrib]
    have eJ : ∏ i, ρ * exp (-z i) = ρ ^ (n : ℝ) * exp (-(∑ i, z i)) := by
      rw [Finset.prod_mul_distrib, Finset.prod_const, Finset.card_univ, Fintype.card_fin,
        ← Real.exp_sum, Finset.sum_neg_distrib, Real.rpow_natCast]
    rw [er, eκ, eJ]
    have e1 : ρ ^ (∑ i, (r i + 1)) = ρ ^ (∑ i, r i) * ρ ^ (n : ℝ) := by
      rw [Finset.sum_add_distrib, Finset.sum_const, Finset.card_univ, Fintype.card_fin,
        nsmul_eq_mul, mul_one, Real.rpow_add hρ]
...
/-- **The constant-unit model kernel in logarithmic coordinates.** -/
theorem modelKernel_const_eq_log {ρ A B D γ p q δ : ℝ} {Q κ r : Fin n → ℝ} {w₀ a₀ t : ℝ}
    (hρ : 0 < ρ) (hD : 0 < D) (hq : 0 < q) (ht : 0 < t) :
    modelKernel ρ A B D γ p q δ Q κ r (fun _ _ ↦ w₀) (fun _ _ ↦ a₀) t =
      A * t ^ (-(γ * p)) * (w₀ * ρ ^ (∑ i, (r i + 1)) *
        ∫ z in Set.pi univ (fun _ : Fin n ↦ Ioi (0 : ℝ)), (logCut ρ D γ q t Q).indicator
          (fun z ↦ exp (-(∑ i, (r i + 1) * z i)) *
            exp (-(B * t ^ δ * a₀ * ρ ^ (∑ i, κ i) * exp (-(∑ i, κ i * z i))))) z) := by
  unfold modelKernel
  have hsupp : ∀ x, modelIntegrand ρ B D γ q δ Q κ r (fun _ _ ↦ w₀) (fun _ _ ↦ a₀) t x ≠ 0 →
      x ∈ Set.pi univ fun _ : Fin n ↦ Ioo (0 : ℝ) ρ := fun x hx ↦ by
    by_contra h
    apply hx
  ...
  refine setIntegral_congr_fun
    (MeasurableSet.univ_pi fun _ : Fin n ↦ (measurableSet_Ioi : MeasurableSet (Ioi (0 : ℝ))))
    fun z hz ↦ ?_
