...
* the differential in a data direction `v` is `−λ ∫ φ U_a^{-λ-1} R_v dν` with `R_v = ∑ vᵢ hᵢ`
  (`FaceData.hasDerivAt_faceCoef`, dominated differentiation; `φ` need only be `ν`-integrable);
...
theorem hasDerivAt_faceCoef [Nonempty ι] [Nonempty Y] (hd : FaceData ν h a c Mh) {φ : Y → ℝ}
    (hφi : Integrable φ ν) {lam : ℝ} (hlam : 0 ≤ lam) (v : ι → ℝ) :
  ...
  have hφm := hφi.aestronglyMeasurable
  have hmeasF : ∀ ε : ℝ, AEStronglyMeasurable
      (fun y ↦ φ y * faceUnit h (a + ε • v) y ^ (-lam)) ν := fun ε ↦
    hφm.mul ((hd.measurable_faceUnit (a + ε • v)).pow_const (-lam)).aestronglyMeasurable
  have hmeasF' : ∀ ε : ℝ, AEStronglyMeasurable (fun y ↦
      φ y * (faceUnit h v y * -lam * faceUnit h (a + ε • v) y ^ (-lam - 1))) ν := fun ε ↦
    hφm.mul (((hd.measurable_faceUnit v).mul_const (-lam)).mul
      ((hd.measurable_faceUnit (a + ε • v)).pow_const (-lam - 1))).aestronglyMeasurable
  have hint : Integrable (fun y ↦ φ y * faceUnit h (a + (0 : ℝ) • v) y ^ (-lam)) ν := by
    refine (hφi.norm.const_mul (c ^ (-lam))).mono' (hmeasF 0)
      (Filter.Eventually.of_forall fun y ↦ ?_)
    simp only [zero_smul, add_zero]
    rw [Real.norm_eq_abs, abs_mul, abs_of_pos (Real.rpow_pos_of_pos (hd.unit_pos y) _),
      Real.norm_eq_abs, mul_comm (c ^ (-lam))]
    exact mul_le_mul_of_nonneg_left (rpow_le_rpow_of_le_of_nonpos hd.c_pos (hd.unit_ge y)
      (neg_nonpos.mpr hlam)) (abs_nonneg _)
  have hbound : ∀ᵐ y ∂ν, ∀ ε ∈ Metric.closedBall (0 : ℝ) (hd.radius v),
      ‖φ y * (faceUnit h v y * -lam * faceUnit h (a + ε • v) y ^ (-lam - 1))‖ ≤
        ‖φ y‖ * (((∑ i, |v i|) * Mh) * lam * (c / 2) ^ (-lam - 1)) := by
    ...
    rw [Real.norm_eq_abs, abs_mul, abs_mul, abs_mul, abs_neg, abs_of_nonneg hlam,
      abs_of_pos (Real.rpow_pos_of_pos (hc2.trans_le hU) _), Real.norm_eq_abs]
    ...
    exact mul_le_mul_of_nonneg_left (mul_le_mul (mul_le_mul_of_nonneg_right h1 hlam) h2 hUp.le
      (mul_nonneg hS hlam)) (abs_nonneg _)
  ...
  have key := hasDerivAt_integral_of_dominated_loc_of_deriv_le
    ...
    hball (Filter.Eventually.of_forall fun ε ↦ hmeasF ε) hint
    (hmeasF' 0) hbound (hφi.norm.mul_const _) hdiff
  have hderiv := key.2
...
theorem hasDerivAt_facePosterior [Nonempty ι] [Nonempty Y] (hd : FaceData ν h a c Mh)
    {φ : Y → ℝ} (hφi : Integrable φ ν) {lam : ℝ} (hlam : 0 ≤ lam) (v : ι → ℝ)
    (hZ : faceCoef ν (fun _ ↦ 1) h lam a ≠ 0) :
    HasDerivAt (fun ε : ℝ ↦ facePosterior ν φ h lam (a + ε • v))
      (-lam * faceCov ν φ (fun y ↦ faceUnit h v y / faceUnit h a y) h lam a) 0 := by
  have := hd.finite
  have hN := hd.hasDerivAt_faceCoef hφi hlam v
  have hZ' := hd.hasDerivAt_faceCoef (φ := fun _ ↦ (1 : ℝ)) (integrable_const _) hlam v
