...
/-- The Euclidean export of a chart system monomialising a truth function `T` over `L'`: the
fields of `WallChartsData` with the truth coordinate `z ℓ` replaced by `T z`. -/
...
/-- Euclidean wall data is chart data for the coordinate truth `z ℓ`. -/
def WallChartsData.toTruth {ℓ : Fin (m + 1)} {L' : Set (Fin (m + 1) → ℝ)}
    (D : WallChartsData m ℓ L') : TruthChartsData m (fun z ↦ z ℓ) L' where
  ι := D.ι
  rep := D.rep
  rep_cont := D.rep_cont
  ρ := D.ρ
  ...
  dom_meas := D.dom_meas
  dens := D.dens
  dens_cont := D.dens_cont
  dens_nonneg := D.dens_nonneg
  dens_supp := D.dens_supp
  S := D.S
  S_ne := D.S_ne
  q := D.q
  k := D.k
...
theorem measurable_chartFun {θ : (Fin (m + 1) → ℝ) → ℝ≥0∞} (hθ : Measurable θ) (i : D.ι) :
  ...
  ((hθ.comp (D.rep_cont i).measurable).mul
    (ENNReal.measurable_ofReal.comp (D.dens_cont i).measurable)).indicator (D.dom_meas i)
...
theorem measurable_totalKernel {θ : (Fin (m + 1) → ℝ) → ℝ≥0∞} (hθ : Measurable θ) :
  ...
  Finset.measurable_sum _ fun i _ ↦
    WallChartsData.measurable_fibreKernel (D.k i) (D.S i) (D.q i) (D.measurable_chartFun hθ i)
...
theorem lintegral_mul_comp_truth (hT : Measurable T) {θ : (Fin (m + 1) → ℝ) → ℝ≥0∞}
  ...
  rw [lintegral_finsetSum (f := fun i s ↦ η s * fibreKernel (D.k i) (D.S i) (D.q i)
    ...
    hη.mul (WallChartsData.measurable_fibreKernel (D.k i) (D.S i) (D.q i)
      (D.measurable_chartFun hθ i))]
