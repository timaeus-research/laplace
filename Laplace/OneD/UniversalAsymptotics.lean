...
private lemma universal_disc : (0 : ℝ) ^ 2 < 3 * 1 * 1 := by simp only [ne_eq, OfNat.ofNat_ne_zero,
                                                               not_false_eq_true, zero_pow, mul_one,
                                                               Nat.ofNat_pos]
...
theorem gibbsExpectation_universal_sq_large_t_rate :
  ...
  obtain ⟨K₀, hK₀_nn, hI0⟩ :=
    I_0_asymptotic (lam := 1) (alpha := 0) (gamma := 1)
      (by exact Real.zero_lt_one) (by exact Real.zero_lt_one) universal_disc
  obtain ⟨K₂, hK₂_nn, hI2⟩ :=
  ...
  have hK₀T_le : K₀ / T ≤ a / 2 := by
    ...
    · rw [div_le_div_iff₀ hT_pos (by exact zero_lt_two : (0:ℝ) < 2)]
  ...
  have hD_pos : 0 < D := by
    exact (pos_iff_pos_of_mul_pos hsqrtTD_pos).mp hsqrt_T_pos
  ...
  have hN_form : T * Real.sqrt T * N = a + ε := Eq.symm (add_sub_cancel a (T * √T * N))
  have hD_form : Real.sqrt T * D = a + δ := Eq.symm (add_sub_cancel a (√T * D))
  rw [hN_form, hD_form]
  have h_aδ_pos : 0 < a + δ := by
    exact lt_of_lt_of_eq hsqrtTD_pos hD_form
  have h_aδ_lb : a / 2 ≤ a + δ := by
    exact le_of_le_of_eq hsqrtTD_lb hD_form
  have h_a2_pos : (0 : ℝ) < a / 2 := half_pos ha_pos
  have hratio_eq : (a + ε) / (a + δ) - 1 = (ε - δ) / (a + δ) := by
    field_simp
    exact add_sub_add_left_eq_sub ε δ a
  ...
  calc |ε - δ| / (a + δ)
    ...
    _ ≤ ((K₀ + K₂) / T) / (a / 2) := by
        have h_num_nn : (0 : ℝ) ≤ (K₀ + K₂) / T :=
          div_nonneg (by exact Left.add_nonneg hK₀_nn hK₂_nn) hT_pos.le
...
theorem gibbsExpectation_universal_sq_tendsto :
  ...
  · -- K = 0 case: bound is trivially 0 ≤ K/T = 0/T = 0 < ε.
    ...
    exact Std.lt_of_le_of_lt hbnd hε
