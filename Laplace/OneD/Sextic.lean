...
theorem sextic_moment_even (n : ℕ) {t : ℝ} (ht : 0 < t) :
  ...
  have h := kth_moment_even (k := 3) (by exact NeZero.one_le) n ht
  exact h
...
theorem sextic_partition {t : ℝ} (ht : 0 < t) :
  ...
  rw [sexticPotential_eq_kthPotential]
  have h := partitionFunction_kthPotential (k := 3) (by exact NeZero.one_le) ht
  exact h
...
theorem sextic_expected_value_even (n : ℕ) {t : ℝ} (ht : 0 < t) :
  ...
  rw [sexticPotential_eq_kthPotential]
  have h := gibbsExpectation_kthPotential_even (k := 3) (by exact NeZero.one_le) n ht
  exact h
