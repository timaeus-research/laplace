...
Review v21's qualification (ii) (coefficients only as limits) is thereby closed: the family coefficients are the paper's explicit absolutely convergent series.
...
Not formalised: the Cauchy-estimate bridge from holomorphy on a polydisc to `∑|c_γ| < ∞`, and the `∂_μ` half of the derivative dictionary (`(−∂_μ)^i` ↔ `(−log t)^i`).
