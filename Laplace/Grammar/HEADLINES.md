...
Hypothesis: `ξ(u) = ∑_γ cξ_γ u^γ`, `η(u) = ∑_γ cη_γ u^γ` with `∑|c_γ| < ∞` (`CoeffFamily.AbsSummable`; implied by the paper's holomorphy on `D_R`, `R > 1`, via Cauchy estimates — that bridge is not formalised). Route: box truncations `truncList c m` (finite `MonoRep`), coefficient stability, pass to the limit at fixed `N`.
| Headline | Statement | File:line |
|---|---|---|
...
Not formalised: `b ≠ 1` (scaling), the Cauchy-estimate bridge from holomorphy on a polydisc to `∑|c_γ| < ∞`, the derivative dictionary.
