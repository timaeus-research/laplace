...
(`exp_gap_isLittleO_powLog`). Establishing the gap `f ≥ ε` on the complement of a neighbourhood of
the zero set is a separate geometric task and is not addressed here.
-/
...
theorem norm_setIntegral_exp_gap_le {X : Type*} [MeasurableSpace X] {μ : Measure X} (S : Set X)
    (hS : MeasurableSet S) (a g f : X → ℝ) (β N ε : ℝ) (hβN : 0 ≤ β * N) (hg : IntegrableOn g S μ)
    (hbound : ∀ x ∈ S, ‖a x‖ ≤ g x) (hgap : ∀ x ∈ S, ε ≤ f x) :
  ...
  refine mul_le_mul (Real.exp_le_exp.2 ?_) (hbound x hx) (norm_nonneg _) (Real.exp_pos _).le
  have := hgap x hx
  nlinarith
...
theorem exp_gap_isBigO {X : Type*} [MeasurableSpace X] {μ : Measure X} (S : Set X)
  ...
  exact (norm_setIntegral_exp_gap_le S hS (a N) g f β N ε (mul_nonneg hβ hN) hg (hbound N)
    hgap).trans (le_of_eq (mul_comm _ _))

/-- `e^{-βNε}` is `o` of every power–log scale `N^{-λ} (log N)^r` when `βε > 0`. -/
