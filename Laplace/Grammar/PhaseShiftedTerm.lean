...
* `phaseShift h k j = fun i => h i + j * k i`; `prod_pow_shiftExp`: `u^{h+jk} = u^h (u^k)^j`.
* `ratioExp_shiftExp`: `ratioExp (h+jk) k i = ratioExp h k i + j/2`; hence `multCount_shiftExp`,
  `faceProj_shiftExp`, `residualWeight_shiftExp` (the residual exponents `hᵢ + jkᵢ − 2kᵢ(λ+j/2)`
  are `hᵢ − 2kᵢλ`), and the explicit `faceLeadConst_shiftExp`, `amplitudeCoeff_shiftExp`.
...
theorem prod_pow_shiftExp {d : ℕ} (h k : Fin d → ℕ) (j : ℕ) (x : Fin d → ℝ) :
...
theorem ratioExp_shiftExp {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (i : Fin d) :
...
theorem ratioExp_shiftExp_eq_iff {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (l : ℝ)
    (i : Fin d) : ratioExp (phaseShift h k j) k i = l + j / 2 ↔ ratioExp h k i = l := by
  rw [ratioExp_shiftExp h k hk j i, add_left_inj]

theorem multCount_shiftExp {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (l : ℝ) :
    multCount (ratioExp (phaseShift h k j) k) (l + j / 2) = multCount (ratioExp h k) l := by
  unfold multCount
  simp only [ratioExp_shiftExp_eq_iff h k hk j l]

theorem faceProj_shiftExp {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (l : ℝ) :
    faceProj (phaseShift h k j) k (l + j / 2) = faceProj h k l := by
  funext u i
  simp only [faceProj, ratioExp_shiftExp_eq_iff h k hk j l]

theorem residualWeight_shiftExp {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (l : ℝ) :
    residualWeight (phaseShift h k j) k (l + j / 2) = residualWeight h k l := by
  funext u
  unfold residualWeight
  refine Finset.prod_congr rfl fun i _ => ?_
  by_cases hi : ratioExp h k i = l
  · rw [if_pos hi, if_pos ((ratioExp_shiftExp_eq_iff h k hk j l i).2 hi)]
  · rw [if_neg hi, if_neg fun h' => hi ((ratioExp_shiftExp_eq_iff h k hk j l i).1 h')]
    congr 1
    simp only [phaseShift]
    push_cast
    ring

theorem faceLeadConst_shiftExp {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (l β : ℝ) :
  ...
  rw [multCount_shiftExp h k hk j l]
  congr 1
  refine Finset.prod_congr rfl fun i _ => ?_
  simp only [ratioExp_shiftExp_eq_iff h k hk j l]

/-- The shifted face functional is the unshifted face integral with the shifted Gamma constant. -/
theorem amplitudeCoeff_shiftExp {d : ℕ} (h k : Fin d → ℕ) (hk : ∀ i, 0 < k i) (j : ℕ) (l β : ℝ)
    (η : (Fin d → ℝ) → ℝ) :
  ...
  rw [faceLeadConst_shiftExp h k hk j l β, faceProj_shiftExp h k hk j l,
    residualWeight_shiftExp h k hk j l]
