...
`Δ(β,n;ξ,η) = O(e^{-εn})` (`b = 1`, `ε = β/8`, `β > 0`), with the main sum a finite combination
of the scales `N^{-μ} (log N)^k`, `μ ∈ {(hᵢ+1)/(2kᵢ)}`, `k < multiplicity`.

**Scope and conventions.** All parameters `n, h, k, β, a, p` are fixed; the implicit constant
depends on them (it contains `(⌈ν⌉+i+p)! (4/β)^{⌈ν⌉+i+p}`), so nothing here licenses summing
infinitely many phase orders or monomials — that uniformity is Stage 3's obligation. Interpretation of the moments:
